...
{
    if (argc < 3) {
        std::cout << "Usage: " << argv[0] << " filexyz.bin fileq.bin e.g.: combxyz.bin combq.bin" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    vtkStructuredGrid *pl3dOutput = dynamic_cast<vtkStructuredGrid *>(pl3d->GetOutput()->GetBlock(0));

    std::vector<vtkSmartPointer<vtkRenderer>> renderers;

    // Create the 4 renderers
    vtkNew<vtkRenderer> grayScaleRenderer;
    renderers.push_back(grayScaleRenderer.Get());
    vtkNew<vtkRenderer> rainbowBlueRedRenderer;
    renderers.push_back(rainbowBlueRedRenderer.Get());
    vtkNew<vtkRenderer> rainbowRedBlueRenderer;
    renderers.push_back(rainbowRedBlueRenderer.Get());
    vtkNew<vtkRenderer> largeContrastRenderer;
    renderers.push_back(largeContrastRenderer.Get());
    ...
    grayScaleLut->Build();
    luts.push_back(grayScaleLut.Get());
    ...
    luts.push_back(rainbowBlueRedLut.Get());
    ...
    luts.push_back(rainbowRedBlueLut.Get());
    ...
    luts.push_back(highContrastLut.Get());
    ...
    for (size_t r = 0; r < renderers.size(); ++r) {
        ...
        planeMapper->SetLookupTable(lut.Get());
        ...
        //
        renderers[r]->AddActor(outlineActor.Get());
        renderers[r]->AddActor(planeActor.Get());

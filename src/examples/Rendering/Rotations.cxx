...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " filename [figure] [actorColor]" << std::endl;
    ...
    modelMapper->SetInputData(polyData.Get());

    vtkNew<vtkActor> modelActor;
    modelActor->SetMapper(modelMapper.Get());
    modelActor->GetProperty()->SetDiffuseColor(colors->GetColor3d(actorColor).GetData());
    ...
    modelAxes->SetMapper(modelAxesMapper.Get());

    ren1->AddActor(modelAxes.Get());
    modelAxes->VisibilityOff();

    // Add the actors to the renderer, set the background and size.
    //
    ren1->AddActor(modelActor.Get());
    if (actorColor == "Wheat") {
    ...
    default :
    case 0 :
        RotateXY(renWin.Get(), modelActor.Get());
        break;
    case 1 :
        RotateX(renWin.Get(), modelActor.Get());
        break;
    case 2 :
        RotateY(renWin.Get(), modelActor.Get());
        break;
    case 3 :
        RotateZ(renWin.Get(), modelActor.Get());
        break;
    }
    renWin->EraseOff();
    iren->Start();

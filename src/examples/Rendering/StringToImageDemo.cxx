...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < tried; ++i) {
        ...
        freeType->RenderString(textProperty.Get(), text, dpi, textImage.GetPointer());

        // Set the extent of the text image
        std::array<int, 4> bb;
        freeType->GetBoundingBox(textProperty.Get(), text, dpi, bb.data());
        ...
        if (offsetX + bb[1] - bb[0] < 639 && offsetY + bb[3] - bb[2] < 479) {
            textImage->SetExtent(offsetX, offsetX + bb[1] - bb[0], offsetY, offsetY + bb[3] - bb[2], 0, 0);
            ...
            if (good) {
                accepted++;
                image->DeepCopy(textImage.Get());
                final->AddInputData(image.Get());
    ...
    imageViewer->SetupInteractor(interactor.Get());

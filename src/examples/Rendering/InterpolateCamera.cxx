...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 0; i < keyPoints.size() + 1; ++i) {
        ...
        interpolator->AddCamera((double)i, cam.Get());
    }
    ...
    for (auto i = 0; i < numSteps; ++i) {
        ...
        interpolator->InterpolateCamera(t, camera.Get());
                      ...
                      std::vector<std::array<double, 3>> &keyPoints)
{
    ...
    double range;
    range = std::max(std::max(bounds[1] - bounds[0], bounds[3] - bounds[2]), bounds[5] - bounds[3]);

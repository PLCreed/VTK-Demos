...
int main(int argc, char *argv[])
{
    ...
    cubeActor->GetProperty()->SetDiffuseColor(colors->GetColor3d("DarkGreen").GetData());
    ...
    coneActor->GetProperty()->SetDiffuseColor(colors->GetColor3d("DarkTurquoise").GetData());
    ...
    iren->AddObserver(vtkCommand::KeyPressEvent, keypressCallback.Get());
...
namespace
{
void KeypressCallbackFunction(vtkObject *caller, long unsigned int vtkNotUsed(eventId), void *vtkNotUsed(clientData),
                              void *vtkNotUsed(callData))
{
    vtkRenderWindowInteractor *iren = static_cast<vtkRenderWindowInteractor *>(caller);
    vtkRendererCollection *renderers = iren->GetRenderWindow()->GetRenderers();
    if (renderers->GetNumberOfItems() < 2) {
        std::cerr << "We need at least two renderers, we have only " << renderers->GetNumberOfItems() << std::endl;
        return;
    }

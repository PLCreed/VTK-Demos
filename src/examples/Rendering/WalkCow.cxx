...
int main(int argc, char *argv[])
{
    ...
    std::array<unsigned char, 4> bkg1 {
        {60, 93, 144, 255}
    };
    colors->SetColor("BkgColor1", bkg1.data());
    std::array<unsigned char, 4> bkg2 {
        {26, 51, 102, 255}
    };
    ...
    cowAxes->SetMapper(cowAxesMapper.Get());
    cowAxes->VisibilityOff();

    ren->AddActor(cowAxes.Get());
    ...
    // modifiedCallback->SetCallback(CameraModifiedCallback.Get());
    // ren->GetActiveCamera()->AddObserver(vtkCommand::ModifiedEvent,
    // modifiedCallback.Get());

    // These four rotations use the same camera position.
    Rotate_X(cowActor.Get(), ren.Get(), renWin.Get());
    Rotate_Y(cowActor.Get(), ren.Get(), renWin.Get());
    Rotate_Z(cowActor.Get(), ren.Get(), renWin.Get());
    Rotate_XY(cowActor.Get(), ren.Get(), renWin.Get());

    ren->SetBackground(colors->GetColor3d("BkgColor2").GetData());
    switch (figure) {
    default :
    case 0 :
        Rotate_V_0(cowActor.Get(), ren.Get(), renWin.Get());
        Rotate_V_V(cowActor.Get(), ren.Get(), renWin.Get());
        // Walk() needs to go after Rotate_V_0() or Rotate_V_V().
        Walk(cowActor.Get(), ren.Get(), renWin.Get());
        break;
    case 1 :
        Rotate_V_0(cowActor.Get(), ren.Get(), renWin.Get());
        break;

...
void MakeWidget(vtkSmartPointer<vtkSliderWidget> &widget, vtkSmartPointer<vtkTessellatorFilter> &tessellate,
                vtkSmartPointer<vtkTextMapper> &textMapper, vtkSmartPointer<vtkRenderWindowInteractor> &interactor);
} // namespace
...
int main(int, char *[])
{
    ...
    vtkSmartPointer<vtkTessellatorFilter> tessellate = vtkSmartPointer<vtkTessellatorFilter>::New();
    tessellate->SetInputData(uGrid.Get());
    ...
    glyph3D->SetInputData(uGrid.Get());
    ...
    glyph3DActor->SetMapper(glyph3DMapper.Get());
    glyph3DActor->GetProperty()->SetColor(namedColors->GetColor3d("Banana").GetData());
    ...
    renderer->AddActor(glyph3DActor.Get());
    renderer->AddViewProp(textActor.Get());
...
{
public:
    static SliderCallbackChordError *New() { return new SliderCallbackChordError; }
                ...
                vtkSmartPointer<vtkTextMapper> &textMapper, vtkSmartPointer<vtkRenderWindowInteractor> &interactor)
{
    ...
    widget->SetInteractor(interactor.Get());
    widget->SetRepresentation(sliderRepChordError.Get());
    ...
    widget->AddObserver(vtkCommand::InteractionEvent, callbackChordError.Get());
}
...
vtkSmartPointer<vtkUnstructuredGrid> MakeQuadraticTetra()
{
    ...
    for (auto i = 0; i < aTetra->GetNumberOfPoints(); ++i) {
        ...
        points->SetPoint(i, *(pcoords + 3 * i) + perturbation[0], *(pcoords + 3 * i + 1) + perturbation[1],

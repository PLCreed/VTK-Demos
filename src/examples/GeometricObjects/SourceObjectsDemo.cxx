...
int main(int, char *[])
{
    ...
    static_cast<vtkTextSource *>(sourceObjects.back().GetPointer())->SetText("Hello");
    ...
    static_cast<vtkPointSource *>(sourceObjects.back().GetPointer())->SetNumberOfPoints(500);

    sourceObjects.push_back(vtkSmartPointer<vtkDiskSource>::New());
    static_cast<vtkDiskSource *>(sourceObjects.back().GetPointer())->SetCircumferentialResolution(51);
    ...
    for (unsigned int i = 0; i < sourceObjects.size(); i++) {
        ...
        actors[i]->SetBackfaceProperty(backProperty.Get());
    ...
    for (auto i = static_cast<int>(sourceObjects.size()); i < gridDimensions * gridDimensions; ++i) {
        renderers.push_back(vtkSmartPointer<vtkRenderer>::New());
    }

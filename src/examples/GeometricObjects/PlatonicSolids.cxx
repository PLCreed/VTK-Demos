 ...
 */
std::vector<std::tuple<std::string, double, double, double>> GetNameOrientation();
 ...
 */
vtkSmartPointer<vtkLookupTable> GetPlatonicLUT();
...
int main(int, char *[])
{
    ...
    for (auto i = 0; i < nameOrientation.size(); ++i) {
        ...
        mappers[i]->SetLookupTable(lut.Get());
        ...
        textMappers[i]->SetTextProperty(textProperty.Get());
    ...
    renWin->SetSize(rendererSize * gridDimensionsX, rendererSize * gridDimensionsY);
    for (auto row = 0; row < gridDimensionsY; ++row) {
        for (auto col = 0; col < gridDimensionsX; ++col) {
            auto index = row * gridDimensionsX + col;

            // (xmin, ymin, xmax, ymax)
            double viewport[4] = {static_cast<double>(col) / gridDimensionsX,
            ...
            renderers[index]->GetActiveCamera()->Azimuth(std::get<1>(nameOrientation[index]));
            renderers[index]->GetActiveCamera()->Elevation(std::get<2>(nameOrientation[index]));
            renderers[index]->GetActiveCamera()->Zoom(std::get<3>(nameOrientation[index]));
        }
    }
...
std::vector<std::tuple<std::string, double, double, double>> GetNameOrientation()
{
    auto getTuple = [](std::string const &name, double const &azimuth, double const &elevation, double const &zoom)
    {

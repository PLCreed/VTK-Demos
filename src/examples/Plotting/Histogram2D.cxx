...
int main(int, char *[])
{
    ...
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            dPtr[i * size + j] =
                std::sin(vtkMath::RadiansFromDegrees(double(2 * i))) * std::cos(vtkMath::RadiansFromDegrees(double(j)));
        }
    }
    chart->SetInputData(data.Get());
    ...
    chart->SetTransferFunction(transferFunction.Get());

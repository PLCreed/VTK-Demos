...
int main(int, char *[])
{
    ...
    arrX0->SetName("X");
    varXSolution->AddColumn(arrX0.Get());

    vtkNew<vtkFloatArray> arrX1;
    arrX1->SetName("Y");
    varXSolution->AddColumn(arrX1.Get());

    vtkNew<vtkFloatArray> arrX2;
    arrX2->SetName("Z");
    varXSolution->AddColumn(arrX2.Get());
    ...
    plot->SetInputData(varXSolution.Get());
#if VTK_HAS_SETCOLORF
    plot->GetPen()->SetColorF(colors->GetColor3d("LightCoral").GetData());
#else
    plot->GetPen()->SetColor(colors->GetColor4ub("LightCoral").GetData());
#endif
    ...
    view->GetRenderer()->SetBackground(colors->GetColor3d("DarkOliveGreen").GetData());

...
int main(int, char *[])
{
    ...
    chartScene->AddItem(chart.Get());
    chartActor->SetScene(chartScene.Get());

    // both needed
    renderer->AddActor(chartActor.Get());
    chartScene->SetRenderer(renderer.Get());
    ...
    dynamic_cast<vtkPlotPoints *>(points)->SetMarkerStyle(vtkPlotPoints::PLUS);
    points = chart->AddPlot(vtkChart::POINTS);
    points->SetInputData(table.Get(), 0, 3);
    points->SetColor(color3d.GetRed(), color3d.GetGreen(), color3d.GetBlue());
    points->SetWidth(1.0);
#endif

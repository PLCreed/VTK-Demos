...
int main(int, char *[])
{
    ...
    for (auto fit = frequencies.begin(); fit != frequencies.end(); fit++) {
        ...
        titleMapper->SetTextProperty(titleProperty.Get());

        vtkNew<vtkActor2D> titleActor;
        titleActor->SetMapper(titleMapper.Get());
        ...
        titleActor->GetProperty()->SetColor(colors->GetColor3d("MidnightBlue").GetData());

        vtkNew<vtkRenderer> renderer;
        renderer->AddActor(chart.Get());
        renderer->AddActor(titleActor.Get());
    ...
    for (auto row = 0; row < gridRows; row++) {
        for (auto col = 0; col < gridCols; col++) {
            ...
            double viewport[4] = {
                static_cast<double>(col) / gridCols, static_cast<double>(gridRows - row - 1) / gridRows,
                static_cast<double>(col + 1) / gridCols, static_cast<double>(gridRows - row) / gridRows};
...
vtkSmartPointer<vtkIntArray> CreateUniformDistribution(int count, double a, double b)
{
    ...
    vtkSmartPointer<vtkIntArray> frequenciesArray = vtkSmartPointer<vtkIntArray>::New();

...
int main(int, char *[])
{
    ...
    for (int i = 0; i < numCols; i++) {
        ...
        inputTable->AddColumn(arr[i].Get());
    }
    ...
    inputTable->AddColumn(q3Arr.Get());
    inputTable->AddColumn(q2Arr.Get());
...
#endif
    q3Plot->SetInputData(inputTable.Get(), "X", "Q3");
    chart->AddPlot(q3Plot.Get());
...
#endif
    q2Plot->SetInputData(inputTable.Get(), "X", "Q2");
    chart->AddPlot(q2Plot.Get());
    ...
    for (int j = 0; j < numCols; j++) {
        ...
        lookup->SetTableValue(j, color.GetRed() / 255., color.GetGreen() / 255., color.GetBlue() / 255., 1.);
        double rgb[3];
...
#endif
        plot->SetInputData(inputTable.Get(), "X", inputTable->GetColumn(j)->GetName());
        plot->GetPen()->SetWidth(3.0);
        chart->AddPlot(plot.Get());
    }

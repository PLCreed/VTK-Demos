...
int main(int, char *[])
{
    ...
    leftChartScene->AddItem(leftChart.Get());
    leftChartActor->SetScene(leftChartScene.Get());

    leftRenderer->AddActor(leftChartActor.Get());
    leftChartScene->SetRenderer(leftRenderer.Get());
...
#else
    leftChart->GetBackgroundBrush()->SetColor(colors->GetColor4ub("MistyRose").GetData());
#endif
    ...
    rightChartScene->AddItem(rightChart.Get());
    rightChartActor->SetScene(rightChartScene.Get());

    rightRenderer->AddActor(rightChartActor.Get());
    rightChartScene->SetRenderer(rightRenderer.Get());
...
#else
    rightChart->GetBackgroundBrush()->SetColor(colors->GetColor4ub("Thistle").GetData());
#endif
    ...
    points->SetInputData(table.Get(), 0, 2);
    points->SetColor(ptColor.GetRed(), ptColor.GetGreen(), ptColor.GetBlue(), ptColor.GetAlpha());
    points->SetWidth(1.0);

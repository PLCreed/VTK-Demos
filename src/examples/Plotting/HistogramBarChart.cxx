...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < numComponents; ++i) {
        ...
        if (i == 0) {
            currentArray = redFrequencies.Get();
        } else if (i == 1) {
            currentArray = greenFrequencies.Get();
        } else {
            currentArray = blueFrequencies.Get();
        }
    ...
    if (numComponents == 1) {
        dataObject->GetFieldData()->AddArray(redFrequencies.Get());
    } else {
        vtkIntArray *rgb[3] = {0, 0, 0};
        vtkNew<vtkIntArray> allFrequencies;
        allFrequencies->SetNumberOfComponents(1);
        if (numComponents == 2) {
            rgb[0] = redFrequencies.Get();
            rgb[1] = greenFrequencies.Get();
        } else {
            rgb[0] = redFrequencies.Get();
            rgb[1] = greenFrequencies.Get();
            rgb[2] = blueFrequencies.Get();
        }
        for (int i = 0; i < 54; ++i) {
        ...
        dataObject->GetFieldData()->AddArray(allFrequencies.Get());
    }
    ...
    // long
    //
    barChart->SetInput(dataObject.Get());
    ...
    renderer->AddActor(barChart.Get());

...
int main(int, char *[])
{
    ...
    dobj->GetFieldData()->AddArray(bitter.Get());
    dobj->GetFieldData()->AddArray(crispy.Get());
    dobj->GetFieldData()->AddArray(crunchy.Get());
    dobj->GetFieldData()->AddArray(salty.Get());
    dobj->GetFieldData()->AddArray(oily.Get());

    vtkNew<vtkSpiderPlotActor> actor;
    actor->SetInputData(dobj.Get());
    ...
    // Set text colors
    actor->GetTitleTextProperty()->SetColor(colors->GetColor3d("MistyRose").GetData());
    actor->GetLabelTextProperty()->SetColor(colors->GetColor3d("MistyRose").GetData());

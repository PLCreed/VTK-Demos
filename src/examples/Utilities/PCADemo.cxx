...
int main(int, char *[])
{
    ...
    datasetTable->AddColumn(xArray.Get());
    datasetTable->AddColumn(yArray.Get());

    vtkNew<vtkPCAStatistics> pcaStatistics;
    pcaStatistics->SetInputData(vtkStatisticsAlgorithm::INPUT_DATA, datasetTable.Get());
    ...
    pcaStatistics->GetEigenvalues(eigenvalues.Get());
    ...
    pcaStatistics->GetEigenvectors(eigenvectors.Get());

    vtkNew<vtkDoubleArray> evec1;
    pcaStatistics->GetEigenvector(0, evec1.Get());

    vtkNew<vtkDoubleArray> evec2;
    pcaStatistics->GetEigenvector(1, evec2.Get());
    ...
    vector1Actor->SetMapper(vec1Mapper.Get());
    ...
    vector2Actor->SetMapper(vec2Mapper.Get());
    ...
    projectedPolyData->SetPoints(projectedPoints.Get());

    vtkNew<vtkVertexGlyphFilter> projectedGlyphFilter;
    projectedGlyphFilter->SetInputData(projectedPolyData.Get());
    ...
    projectedActor->SetMapper(projectedMapper.Get());
    ...
    auto sphereActor = PointToGlyph(transformFilter->GetOutput()->GetPoints(), 0.007);
    sphereActor->GetProperty()->SetColor(colors->GetColor3d("Blue").GetData());

    // leftRenderer->AddActor(originalActor.Get());
    leftRenderer->AddActor(sphereActor.Get());
    leftRenderer->AddActor(vector1Actor.Get());
    leftRenderer->AddActor(vector2Actor.Get());

    rightRenderer->AddActor(projectedActor.Get());

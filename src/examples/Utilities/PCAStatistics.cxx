...
int main(int, char *[])
{
    ...
    datasetTable->AddColumn(dataset1Arr.Get());
    datasetTable->AddColumn(dataset2Arr.Get());
    datasetTable->AddColumn(dataset3Arr.Get());
    ...
    for (vtkIdType i = 0; i < eigenvectors->GetNumberOfTuples(); i++) {
        ...
        for (vtkIdType j = 0; j < eigenvectors->GetNumberOfComponents(); j++) {
            ...
            pcaStatistics->GetEigenvector(i, eigenvectorSingle.Get());
        }
        delete[] evec;
        std::cout << std::endl;
    }

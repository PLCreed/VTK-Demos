...
void RestoreSceneFromFieldData(vtkDataSet *data, vtkActor *actor, vtkCamera *camera);
} // namespace
...
int main(int argc, char *argv[])
{
    ...
    SaveSceneToFieldData(polyData, actor.Get(), renderer->GetActiveCamera());
    renderWindow->Render();
    renderWindowInteractor->Start();

    // After interaction , restore the scene
    RestoreSceneFromFieldData(polyData, actor.Get(), renderer->GetActiveCamera());
    renderWindow->Render();
    renderWindowInteractor->Start();
...
void SaveSceneToFieldData(vtkDataSet *data, vtkActor * /* actor */, vtkCamera *camera)
{
    ...
    buffer << "Camera:FocalPoint " << vector[0] << ", " << vector[1] << ", " << vector[2] << std::endl;
    camera->GetPosition(vector);
    buffer << "Camera:Position " << vector[0] << ", " << vector[1] << ", " << vector[2] << std::endl;
    camera->GetViewUp(vector);
    buffer << "Camera:ViewUp " << vector[0] << ", " << vector[1] << ", " << vector[2] << std::endl;
    scalar = camera->GetViewAngle();
    buffer << "Camera:ViewAngle " << scalar << std::endl;
    camera->GetClippingRange(vector);
    buffer << "Camera:ClippingRange " << vector[0] << ", " << vector[1] << std::endl;
    ...
    data->GetFieldData()->AddArray(cameraArray.Get());
}
...
void RestoreSceneFromFieldData(vtkDataSet *data, vtkActor * /* actor */, vtkCamera *camera)
{
    ...
    if (dynamic_cast<vtkStringArray *>(data->GetFieldData()->GetAbstractArray("Camera"))) {
        buffer.str(dynamic_cast<vtkStringArray *>(data->GetFieldData()->GetAbstractArray("Camera"))->GetValue(0));
    } else {
        return;
    }
    ...
    vtksys::RegularExpression floatNumber("[^0-9\\.\\-]*([0-9e\\.\\-]*[^,])[^0-9\\.\\-]*([0-9e\\.\\-]*[^,])[^0-9\\."
                                          "\\-]*([0-9e\\.\\-]*[^,])");
    ...
    while (std::getline(buffer, line)) {
        if (reCFP.find(line)) {
            ...
            if (floatNumber.find(rest)) {
                camera->SetFocalPoint(atof(floatNumber.match(1).c_str()), atof(floatNumber.match(2).c_str()),
                                      atof(floatNumber.match(3).c_str()));
            }
        } else if (reCP.find(line)) {
            std::string rest(line, reCP.end());
            if (floatNumber.find(rest)) {
                camera->SetPosition(atof(floatNumber.match(1).c_str()), atof(floatNumber.match(2).c_str()),
                                    atof(floatNumber.match(3).c_str()));
            }
        } else if (reCVU.find(line)) {
            std::string rest(line, reCVU.end());
            if (floatNumber.find(rest)) {
                camera->SetViewUp(atof(floatNumber.match(1).c_str()), atof(floatNumber.match(2).c_str()),
                                  atof(floatNumber.match(3).c_str()));
            }
            ...
            if (floatNumber.find(rest)) {
                camera->SetClippingRange(atof(floatNumber.match(1).c_str()), atof(floatNumber.match(2).c_str()));
            }
        } else {

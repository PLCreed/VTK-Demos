 ...
 */
std::string CommaSeparatedList(std::vector<double> const &v, const int precision = 6, const int width = 10);
...
int main(int, char *[])
{
    ...
    renWin->Render();
    Orientation originalOrient = GetOrientation(ren.Get());
    std::cout << std::setw(23) << "Original orientation:" << CommaSeparatedList(originalOrient.orientation)
              << std::endl;
    ...
    renWin->Render();
    Orientation newOrient = GetOrientation(ren.Get());
    std::cout << std::setw(23) << "New orientation:" << CommaSeparatedList(newOrient.orientation) << std::endl;
    std::this_thread::sleep_for(std::chrono::seconds(1));

    std::cout << "Reloading the original orientation." << std::endl;
    SetOrientation(ren.Get(), originalOrient);
    renWin->Render();
    Orientation check = GetOrientation(ren.Get());
    std::cout << std::setw(23) << "Final orientation:" << CommaSeparatedList(check.orientation) << std::endl;
    std::this_thread::sleep_for(std::chrono::seconds(1));
...
namespace
{
std::string CommaSeparatedList(std::vector<double> const &v, const int precision, const int width)
{
    std::ostringstream os;
    for (auto p = v.begin(); p < v.end() - 1; ++p) {
        os << std::fixed << std::setw(width) << std::setprecision(precision) << *p << ", ";
    }
    os << std::fixed << std::setw(width) << std::setprecision(precision) << v.back();
    return os.str();
}

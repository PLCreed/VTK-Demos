...
int main(int, char *[])
{
    ...
    calc1->SetInputData(polydata.Get());
    calc1->AddScalarArrayName("orig");
    calc1->SetFunction("orig+1");
    calc1->SetResultArrayName("orig");
    calc1->Update();

    auto output1 = dynamic_cast<vtkDoubleArray *>(calc1->GetPolyDataOutput()->GetPointData()->GetArray("orig"));
    ...
    calc2->SetInputData(polydata.Get());
    calc2->AddScalarArrayName("orig");
    calc2->SetFunction("if(orig=2,1,orig)");
    calc2->SetResultArrayName("new");
    calc2->Update();

    auto output2 = dynamic_cast<vtkDoubleArray *>(calc2->GetPolyDataOutput()->GetPointData()->GetArray("new"));

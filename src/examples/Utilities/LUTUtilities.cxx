...
public:
    ...
    std::string DisplayLUTAsString(vtkLookupTable *lut)
    {
        ...
        if (lut->GetIndexedLookup()) {
            ...
            os << "Categorical Lookup Table\nNumber of annotated values: " << av << " Number of table values: " << tv
               << "\nTable Range: " << std::fixed << std::setw(8) << std::setprecision(6) << dR[0] << " to " << dR[1]
               << std::endl;
            if (av > 0) {
    ...
    std::pair<bool, std::string> CompareLUTs(vtkLookupTable *lut1, vtkLookupTable *lut2)
    {
        ...
        if (lut1->GetIndexedLookup() && lut1->GetNumberOfAnnotatedValues() != lut2->GetNumberOfAnnotatedValues()) {
            res.first = false;
...
{
    std::ostringstream os;
    for (std::map<int, std::string>::const_iterator p = colorSchemes.begin(); p != colorSchemes.end(); ++p) {
 ...
 */
void DisplayResults(std::string &reason, vtkLookupTable *lut1, vtkLookupTable *lut2)
{
 ...
 * @return true/false.
 */
bool TestTables(vtkLookupTable *lut1, vtkLookupTable *lut2, bool const expected = true)
{
    LUTUtilities lutUtilities;
...
bool TestLookupTables(bool const &lutMode)
{
    ...
    colorSeries->BuildLookupTable(lut1.Get());
    colorSeries->BuildLookupTable(lut2.Get());
    if (lutMode) {
    ...
    // Are they the same?
    bool res = true;
    res &= TestTables(lut1.Get(), lut2.Get());

    // Different size
    lut2->SetNumberOfTableValues(5);
    res &= TestTables(lut1.Get(), lut2.Get(), false);
    ...
    if (lutMode) {
        ...
        colorSeries->BuildLookupTable(lut3.Get());
        lut3->IndexedLookupOff();
        res &= TestTables(lut1.Get(), lut3.Get(), false);

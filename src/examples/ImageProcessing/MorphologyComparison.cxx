...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::cout << "Usage: " << argv[0] << " Filename e.g. binary.pgm" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    connectedRenderer->AddActor(connectedActor.Get());
    vtkNew<vtkRenderer> dilateRenderer;
    dilateRenderer->AddActor(dilateActor.Get());
    vtkNew<vtkRenderer> erodeRenderer;
    erodeRenderer->AddActor(erodeActor.Get());
    vtkNew<vtkRenderer> closingRenderer;
    closingRenderer->AddActor(closingActor.Get());
    vtkNew<vtkRenderer> openingRenderer;
    openingRenderer->AddActor(openingActor.Get());

    std::vector<vtkSmartPointer<vtkRenderer>> renderers;
    renderers.push_back(originalRenderer.Get());
    renderers.push_back(connectedRenderer.Get());
    renderers.push_back(erodeRenderer.Get());
    renderers.push_back(dilateRenderer.Get());
    renderers.push_back(openingRenderer.Get());
    renderers.push_back(closingRenderer.Get());

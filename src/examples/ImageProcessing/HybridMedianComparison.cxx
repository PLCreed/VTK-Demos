...
namespace
{
void AddShotNoise(vtkImageData *inputImage, vtkImageData *outputImage, double noiseAmplitude, double noiseFraction,
                  int extent[6]);
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::cout << "Usage: " << argv[0] << " Filename e.g. TestPattern.png" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    std::cout << "Range: " << scalarRange[0] << ", " << scalarRange[1] << std::endl;
    int middleSlice = (reader->GetOutput()->GetExtent()[5] - reader->GetOutput()->GetExtent()[4]) / 2;
    ...
    AddShotNoise(originalData.Get(), noisyData.Get(), 2000.0, .1, reader->GetOutput()->GetExtent());
    vtkNew<vtkImageMedian3D> median;
    median->SetInputData(noisyData.Get());
    median->SetKernelSize(5, 5, 1);

    vtkNew<vtkImageHybridMedian2D> hybridMedian1;
    hybridMedian1->SetInputData(noisyData.Get());
    ...
    originalActor->GetMapper()->SetInputData(originalData.Get());
    ...
    noisyActor->GetMapper()->SetInputData(noisyData.Get());
    ...
    noisyRenderer->AddActor(noisyActor.Get());
    vtkNew<vtkRenderer> hybridRenderer;
    hybridRenderer->AddActor(hybridMedianActor.Get());
    vtkNew<vtkRenderer> medianRenderer;
    medianRenderer->AddActor(medianActor.Get());

    std::vector<vtkSmartPointer<vtkRenderer>> renderers;
    renderers.push_back(originalRenderer.Get());
    renderers.push_back(noisyRenderer.Get());
    renderers.push_back(hybridRenderer.Get());
    renderers.push_back(medianRenderer.Get());

...
int main(int argc, char *argv[])
{
    ...
    kdTree->SetDataSet(polyData.Get());
    kdTree->SetMaxLevel(maxLevel);
    kdTree->BuildLocator();

    // Initialize the representation
    vtkNew<vtkPolyData> polydata;
    kdTree->GenerateRepresentation(maxLevel / 2, polydata.Get());

    vtkNew<vtkPolyDataMapper> octreeMapper;
    octreeMapper->SetInputData(polydata.Get());

    vtkNew<vtkActor> octreeActor;
    octreeActor->SetMapper(octreeMapper.Get());
    ...
    octreeActor->GetProperty()->SetColor(colors->GetColor4d("SpringGreen").GetData());
    ...
    renderer->AddActor(octreeActor.Get());

...
double TimeKDTree(vtkPolyData *polydata, int maxPoints, int numberOfTrials, vtkMinimalStandardRandomSequence *rng);
} // namespace
...
int main(int, char *[])
{
    ...
    for (int i = 1; i < 20; i++) {
        double t = TimeKDTree(reader->GetOutput(), i, numberOfTrials, rng.Get());
...
double TimeKDTree(vtkPolyData *polydata, int maxLevel, int numberOfTrials, vtkMinimalStandardRandomSequence *rng)
{

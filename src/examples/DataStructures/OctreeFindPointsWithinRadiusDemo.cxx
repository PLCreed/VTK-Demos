...
TStream &join(TStream &stream, TContainer const &cont, TSeparator const &separator)
{
    auto sep = false;
...
{
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " dragon.ply [number of radii] " << std::endl;
        return EXIT_FAILURE;
    }
    ...
    sphereSource->SetCenter(range[0] / 2.0 + bounds[0], range[1] / 2.0 + bounds[2], bounds[5]);
    ...
    pointTree->SetDataSet(polyData.Get());
    ...
    for (std::vector<double>::reverse_iterator rIter = radii.rbegin(); rIter != radii.rend(); ++rIter) {
        vtkNew<vtkIdList> result;
        pointTree->FindPointsWithinRadius(*rIter, sphereSource->GetCenter(), result.Get());
        vtkIdType k = result->GetNumberOfIds();
        std::cout << k << " points within " << *rIter << " of " << sphereSource->GetCenter()[0] << ", "
                  << sphereSource->GetCenter()[1] << ", " << sphereSource->GetCenter()[2] << std::endl;
    ...
    polyData->GetPointData()->SetScalars(scalars.Get());
    ...
    if (radii.size() < 6) {
        for (std::vector<double>::reverse_iterator rIter = radii.rbegin(); rIter != radii.rend(); ++rIter) {
            ...
            radiiSource->SetCenter(range[0] / 2.0 + bounds[0], range[1] / 2.0 + bounds[2], bounds[5]);
            ...
            radiiActor->SetMapper(radiiMapper.Get());
            radiiActor->GetProperty()->SetDiffuseColor(colors->GetColor3d("White").GetData());
            radiiActor->GetProperty()->SetOpacity(.1);
            radiiActor->SetBackfaceProperty(backProp.Get());

            renderer->AddActor(radiiActor.Get());
        }
    }

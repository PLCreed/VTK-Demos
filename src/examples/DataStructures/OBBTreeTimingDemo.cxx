...
void RandomLineThroughVolume(vtkPolyData *polydata, double p1[3], double p2[3], vtkMinimalStandardRandomSequence *rng);

double TimeOBBTree(vtkPolyData *polydata, int maxPoints, int numberOfTrials, vtkMinimalStandardRandomSequence *rng);
...
int main(int, char *[])
{
    ...
    for (int i = 1; i < 20; i++) {
        double t = TimeOBBTree(reader->GetOutput(), i, numberOfTrials, rng.Get());
...
double TimeOBBTree(vtkPolyData *polydata, int maxLevel, int numberOfTrials, vtkMinimalStandardRandomSequence *rng)
{

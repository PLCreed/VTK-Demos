...
int main(int argc, char *argv[])
{
    ...
    pointsMapper->SetInputData(polyData.Get());
    ...
    bspTree->SetDataSet(polyData.Get());
    ...
    bspTree->GenerateRepresentation(0, polydata.Get());

    vtkNew<vtkPolyDataMapper> bspTreeMapper;
    bspTreeMapper->SetInputData(polydata.Get());

    vtkNew<vtkActor> bspTreeActor;
    bspTreeActor->SetMapper(bspTreeMapper.Get());
    ...
    bspTreeActor->GetProperty()->SetColor(colors->GetColor4d("SpringGreen").GetData());
    ...
    renderer->AddActor(bspTreeActor.Get());
    ...
    callback->BSPTree = bspTree.Get();
    callback->PolyData = polydata.Get();
    callback->Renderer = renderer.Get();

    callback->Execute(sliderWidget.Get(), 0, 0);

...
int main(int, char *[])
{
    ...
    CreateColorImage(colorImage.Get()); // This image has (0,0) at the bottom left corner

    int extent[6];
    colorImage->GetExtent(extent);
    std::cout << "Old extent: " << extent[0] << " " << extent[1] << " " << extent[2] << " " << extent[3] << " "
    ...
    translateExtent->SetInputData(colorImage.Get());
    translateExtent->Update();
    colorImage->DeepCopy(translateExtent->GetOutput());

    colorImage->GetExtent(extent);
    std::cout << "New extent: " << extent[0] << " " << extent[1] << " " << extent[2] << " " << extent[3] << " "
              << extent[4] << " " << extent[5] << std::endl;
    ...
    imageSliceMapper->SetInputData(colorImage.Get());

    vtkNew<vtkImageSlice> imageSlice;
    imageSlice->SetMapper(imageSliceMapper.Get());
    imageSlice->SetPosition(0, 0, 0);

    // Setup renderers
    vtkNew<vtkRenderer> renderer;
    renderer->AddViewProp(imageSlice.Get());
    renderer->AddViewProp(sphereActor.Get());

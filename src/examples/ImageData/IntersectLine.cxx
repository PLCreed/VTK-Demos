...
int main(int, char *[])
{
    ...
    std::cout << "There are " << grid->GetNumberOfPoints() << " points before shrinking." << std::endl;
    std::cout << "There are " << grid->GetNumberOfCells() << " cells before shrinking." << std::endl;
    ...
    for (size_t i = 0; i < 3; ++i) {
        rayEndPoint[i] =
            rayOrigin[i] + rayDirection[i] * 1000; // 1000 is an arbitrary constant that should be much larger
                                                   // than the size of the scene (to create an "infinite" ray)
    }
    std::vector<vtkIdType> intersectedCells = IntersectImage(grid.Get(), rayOrigin, rayEndPoint);

    vtkNew<vtkShrinkFilter> shrinkFilter;
    shrinkFilter->SetInputData(grid.Get());
    ...
    shrinkFilter->GetOutput()->GetCellData()->AddArray(visibilityArray.Get());
    ...
    std::cout << "There are " << thresholdedPolydata->GetNumberOfCells() << " cells after thresholding." << std::endl;
...
std::vector<vtkIdType> IntersectImage(vtkImageData *image, double p0[3], double p1[3])
{
    ...
    int hit = vtkBox::IntersectWithLine(bounds, p0, p1, t0, t1, entrancePoint, exitPoint, *entryPlane, *exitPlane);
    ...
    if (!hit) {
        ...
        std::cout << "Entry point: " << entrancePoint[0] << " " << entrancePoint[1] << " " << entrancePoint[2]
                  << std::endl;

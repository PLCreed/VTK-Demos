...
public:
    ...
    virtual void Execute(vtkObject *caller, unsigned long vtkNotUsed(eventId), void *vtkNotUsed(callData))
    {
        ...
        double pos[3] = {0, 0, 0};
        pos[0] = distance * cos(vtkMath::RadiansFromDegrees(heading)) * cos(vtkMath::RadiansFromDegrees(tilt));
        pos[1] = distance * sin(vtkMath::RadiansFromDegrees(heading)) * cos(vtkMath::RadiansFromDegrees(tilt));
...
int main(int, char *[])
{
    ...
    actor->GetTextEdgesProperty()->SetColor(colors->GetColor3d("Black").GetData());
    ...
    compassWidget->SetInteractor(renderWindowInteractor.Get());
    compassWidget->SetRepresentation(compassRepresentation.Get());
    compassWidget->SetDistance(5.0);

    // create the callback
    vtkNew<vtkICWValueChangedCallback> valueChangedCallback;
    compassWidget->AddObserver(vtkCommand::WidgetValueChangedEvent, valueChangedCallback.Get());

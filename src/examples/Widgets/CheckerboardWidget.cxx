...
int main(int argc, char *argv[])
{
    ...
    checkerWidget->SetInteractor(iren.Get());

    vtkCheckerboardRepresentation *checkerWidgetRep =
        static_cast<vtkCheckerboardRepresentation *>(checkerWidget->GetRepresentation());

    checkerWidgetRep->SetImageActor(checkerActor.Get());
    checkerWidgetRep->SetCheckerboard(checker.Get());

    // Add the actors to the renderer, set the background and size
    //
    ren1->AddActor(checkerActor.Get());

...
{
public:
    static vtkSeedImageCallback *New() { return new vtkSeedImageCallback; }
    ...
    virtual void Execute(vtkObject *, unsigned long event, void *calldata)
    {
        if (event == vtkCommand::PlacePointEvent) {
            ...
            for (unsigned int seedId = 0; static_cast<int>(seedId) < this->SeedRepresentation->GetNumberOfSeeds();
                 seedId++) {
                double pos[3];
                this->SeedRepresentation->GetSeedDisplayPosition(seedId, pos);
                std::cout << "Seed " << seedId << " : (" << pos[0] << " " << pos[1] << " " << pos[2] << ")"
                          << std::endl;
            }
            return;
        }
...
int main(int /* argc */, char * /* argv */[])
{
    ...
    renderWindowInteractor->SetInteractorStyle(interactorStyleImage.Get());

    // Create the representation
    vtkNew<vtkPointHandleRepresentation2D> handle;
    handle->GetProperty()->SetColor(1, 0, 0);
    vtkNew<vtkSeedRepresentation> rep;
    rep->SetHandleRepresentation(handle.Get());

    // Seed widget
    vtkNew<vtkSeedWidget> seedWidget;
    seedWidget->SetInteractor(renderWindowInteractor.Get());
    seedWidget->SetRepresentation(rep.Get());

    vtkNew<vtkSeedImageCallback> seedCallback;
    seedCallback->SetRepresentation(rep.Get());
    seedCallback->SetWidget(seedWidget.Get());
    seedWidget->AddObserver(vtkCommand::PlacePointEvent, seedCallback.Get());
    seedWidget->AddObserver(vtkCommand::InteractionEvent, seedCallback.Get());

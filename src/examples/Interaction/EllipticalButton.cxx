...
public:
    ...
    virtual void Execute(vtkObject *caller, unsigned long, void *)
    {
        ...
        auto rep = reinterpret_cast<vtkProp3DButtonRepresentation *>(buttonWidget->GetRepresentation());
        int state = rep->GetState();
        this->Actor->GetProperty()->SetColor(
            reinterpret_cast<vtkActor *>(rep->GetButtonProp(state))->GetProperty()->GetColor());
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " imageFilename e.g. YinYang.png" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    callback->Actor = actor.Get();
    actor->GetProperty()->SetColor(
        reinterpret_cast<vtkActor *>(buttonRepresentation->GetButtonProp(0))->GetProperty()->GetColor());

    renderWindowInteractor->Initialize();

    vtkNew<vtkButtonWidget> buttonWidget;
    buttonWidget->SetInteractor(renderWindowInteractor.Get());
    buttonWidget->SetRepresentation(buttonRepresentation.Get());
    buttonWidget->AddObserver(vtkCommand::StateChangedEvent, callback.Get());
...
vtkSmartPointer<vtkActor> CreateButtonActor(const char *textureFile)
{
    ...
    vtkSmartPointer<vtkActor> buttonActor = vtkSmartPointer<vtkActor>::New();
    buttonActor->SetMapper(buttonMapper.Get());
    buttonActor->SetTexture(texture.Get());

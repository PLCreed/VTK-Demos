...
public:
    ...
    virtual void Execute(vtkObject *caller, unsigned long vtkNotUsed(eventId), void *vtkNotUsed(callData))
    {
        ...
        if (TestFilter->Counter > 10) {
            std::cout << "Timer Destroyed: " << iren->DestroyTimer(this->timerId) << endl;
        }
    }
...
int main(int, char *[])
{
    ...
    testFilter->AddObserver(testFilter->RefreshEvent, callback.Get());
    testFilter->Update();
    ...
    timerCallback->TestFilter = testFilter.Get();
...
namespace
{
void CallbackFunction(vtkObject *vtkNotUsed(caller), long unsigned int vtkNotUsed(eventId),

...
public:
    ...
    static vtkBorderCallback2 *New() { return new vtkBorderCallback2; }
    ...
    virtual void Execute(vtkObject *caller, unsigned long, void *)
    {
        ...
        std::cout << "Lower left coordinate: " << lowerLeft[0] << ", " << lowerLeft[1] << ", " << lowerLeft[2]
                  << std::endl;
        ...
        std::cout << "Upper right coordinate: " << upperRight[0] << ", " << upperRight[1] << ", " << upperRight[2]
                  << std::endl;
        ...
        if ((lowerLeft[0] > xmin) && (upperRight[0] < xmax) && (lowerLeft[1] > ymin) && (upperRight[1] < ymax)) {
            this->ClipFilter->SetOutputWholeExtent(vtkMath::Round(lowerLeft[0]), vtkMath::Round(upperRight[0]),
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::cerr << "Usage: " << argv[0] << " Filename(.jpg) e.g. Gourds2.jpg" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    borderWidget->SetInteractor(interactor.Get());
    ...
    leftRenderer->AddActor(imageActor.Get());
    ...
    imageClip->SetOutputWholeExtent(
        jPEGReader->GetOutputInformation(0)->Get(vtkStreamingDemandDrivenPipeline::WHOLE_EXTENT()));
    imageClip->ClipDataOn();

    vtkNew<vtkImageActor> clipActor;
    clipActor->GetMapper()->SetInputConnection(imageClip->GetOutputPort());

    rightRenderer->AddActor(clipActor.Get());

    vtkNew<vtkBorderCallback2> borderCallback;
    borderCallback->SetLeftRenderer(leftRenderer.Get());
    borderCallback->SetImageActor(imageActor.Get());
    borderCallback->SetClipFilter(imageClip.Get());

    borderWidget->AddObserver(vtkCommand::InteractionEvent, borderCallback.Get());

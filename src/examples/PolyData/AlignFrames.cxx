...
struct Frame
{
    Frame(float o[3], float x[3], float y[3], float z[3])
    {
        ...
        std::cout << "xDirection: " << this->xDirection[0] << " " << this->xDirection[1] << " " << this->xDirection[2]
                  << std::endl;
        std::cout << "yDirection: " << this->yDirection[0] << " " << this->yDirection[1] << " " << this->yDirection[2]
                  << std::endl;
        std::cout << "zDirection: " << this->zDirection[0] << " " << this->zDirection[1] << " " << this->zDirection[2]
                  << std::endl;
    }
    ...
    void Write(std::string filename)
    {
        ...
        vtkNew<vtkXMLPolyDataWriter> writer;
        writer->SetFileName(filename.c_str());
        writer->SetInputData(polydata.Get());
        writer->Write();
    }
...
int main(int, char *[])
{
    ...
    AlignFrames(frame2, frame1, transform.Get()); // Brings frame2 to frame1

    // std::cout << *transform << std::endl;

    frame2.ApplyTransform(transform.Get(), "transformed.vtp");
...
void AlignFrames(Frame sourceFrame, Frame targetFrame, vtkTransform *transform)
{
    ...
    landmarkTransform->SetSourceLandmarks(sourcePoints.Get());
    landmarkTransform->SetTargetLandmarks(targetPoints.Get());

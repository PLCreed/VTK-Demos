...
int main(int, char *[])
{
    ...
    for (int i = 0; i < xSize; i++) {
        ...
        std::cout << "(y,z) value of point " << i << " : (" << yval << " , " << zval << ")" << std::endl;
    ...
    cells->InsertNextCell(xPolyLine.Get());

    // Create a polydata to store everything in
    vtkNew<vtkPolyData> polyData;

    // Add the points to the dataset
    polyData->SetPoints(xHullPoints.Get());
    ...
    xHullActor->SetMapper(xHullMapper.Get());
    ...
    renderer->AddActor(xHullActor.Get());

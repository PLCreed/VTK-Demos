...
int main(int, char *[])
{
    ...
    for (unsigned int i = 0; i < numberofpoints; i++) {
        const double angle = 2.0 * vtkMath::Pi() * static_cast<double>(i) / static_cast<double>(numberofpoints);
    ...
    circle->SetPoints(points.Get());
    circle->SetLines(lines.Get());

    vtkNew<vtkPolyDataMapper> c_mapper;
    c_mapper->SetInputData(circle.Get());

    vtkNew<vtkNamedColors> colors;

    vtkNew<vtkActor> c_actor;
    c_actor->SetMapper(c_mapper.Get());
    ...
    decimate->SetInputData(circle.Get());

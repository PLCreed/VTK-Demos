...
int main(int argc, char *argv[])
{
    ...
    mapper_data->SetInputData(data3d.Get());

    vtkNew<vtkActor> actor_data;
    actor_data->SetMapper(mapper_data.Get());
    ...
    tmp_rend->AddActor(actor_data.Get());
    ...
    tmp_rW->AddRenderer(tmp_rend.Get());

    tmp_rW->Render();

    // Get a print of the window
    vtkNew<vtkWindowToImageFilter> windowToImageFilter;
    windowToImageFilter->SetInput(tmp_rW.Get());
    ...
    tfilter1->SetInputData(contour.Get());
    tfilter1->SetTransform(transform1.Get());
    tfilter1->Update();
    ...
    tfilter2->SetInputData(contour.Get());
    tfilter2->SetTransform(transform2.Get());
    tfilter2->Update();
    ...
    actor_data->GetProperty()->SetColor(colors->GetColor3d("MistyRose").GetData());

    // Create a mapper and actor of the silhouette
    vtkNew<vtkPolyDataMapper> mapper_contour;
    mapper_contour->SetInputData(contour.Get());

    vtkNew<vtkActor> actor_contour;
    actor_contour->SetMapper(mapper_contour.Get());
    actor_contour->GetProperty()->SetLineWidth(2.0);

    // 2 renderers and a render window
    vtkNew<vtkRenderer> renderer1;
    renderer1->AddActor(actor_data.Get());

    vtkNew<vtkRenderer> renderer2;
    renderer2->AddActor(actor_contour.Get());

    vtkNew<vtkRenderWindow> renderwindow;
    renderwindow->SetSize(400, 400);
    renderwindow->SetWindowName("ExternalContour");
    renderwindow->AddRenderer(renderer1.Get());

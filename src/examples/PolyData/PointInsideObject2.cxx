...
int main(int, char *[])
{
    ...
    bigSphereActor->SetMapper(bigSphereMapper.Get());
    bigSphereActor->GetProperty()->SetOpacity(0.5);
    bigSphereActor->GetProperty()->SetColor(colors->GetColor3d("MistyRose").GetData());
    ...
    smallSphereActor->SetMapper(smallSphereMapper.Get());
    smallSphereActor->GetProperty()->SetColor(colors->GetColor3d("Cornsilk").GetData());
    ...
    for (vtkIdType i = 0; i < smallSphereSource->GetOutput()->GetNumberOfPoints(); i++) {
        ...
        double weights[4];
        int subId;
        vtkIdType cellId = delaunay3D->GetOutput()->FindCell(point, nullptr, 0, 0.1, subId, pcoords, weights);
    ...
    polyDataInside->SetPoints(pointsInside.Get());

    vtkNew<vtkVertexGlyphFilter> vertexGlyphFilter;
    vertexGlyphFilter->SetInputData(polyDataInside.Get());
    ...
    pointsInsideActor->SetMapper(pointsInsideMapper.Get());
    pointsInsideActor->GetProperty()->SetPointSize(6);
    pointsInsideActor->GetProperty()->SetColor(colors->GetColor3d("Magenta").GetData());
    ...
    renderer->AddActor(smallSphereActor.Get());
    renderer->AddActor(bigSphereActor.Get());
    renderer->AddActor(pointsInsideActor.Get());

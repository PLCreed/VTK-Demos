...
int main(int, char *[])
{
    ...
    for (auto i = 0; i < probeTerrain->GetOutput()->GetNumberOfPoints(); ++i) {
        ...
        if (cellLocator->IntersectWithLine(rayStart, rayEnd, 0.0001, t, xyz, pcoords, subId)) {
    ...
    originalActor->GetProperty()->SetColor(colors->GetColor3d("Seashell").GetData());
    ...
    resampledActor->SetMapper(resampledMapper.Get());
    resampledActor->GetProperty()->SetRepresentationToWireframe();
    resampledActor->GetProperty()->SetColor(colors->GetColor3d("Seashell").GetData());

    double rightViewport[4] = {0.5, 0.0, 1.0, 1.0};
    vtkNew<vtkRenderer> rightRenderer;
    rightRenderer->SetViewport(rightViewport);
    rightRenderer->UseHiddenLineRemovalOn();
    rightRenderer->AddActor(resampledActor.Get());

...
int main(int, char *[])
{
    ...
    std::cout << "There are " << pointSource->GetNumberOfPoints() << " points." << std::endl;
    ...
    std::cout << "There are " << quantized->GetNumberOfPoints() << " quantized points." << std::endl;
    ...
    for (vtkIdType i = 0; i < pointSource->GetOutput()->GetNumberOfPoints(); i++) {
        ...
        std::cout << "Point " << i << " : (" << pOrig[0] << ", " << pOrig[1] << ", " << pOrig[2] << ")"
                  << " (" << pQuantized[0] << ", " << pQuantized[1] << ", " << pQuantized[2] << ")" << std::endl;
    }
    ...
    quantizedActor->SetMapper(quantizedMapper.Get());
    quantizedActor->GetProperty()->SetColor(colors->GetColor3d("Orchid").GetData());
    ...
    rightRenderer->AddActor(quantizedActor.Get());

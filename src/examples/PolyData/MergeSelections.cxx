...
int main(int, char *[])
{
    ...
    selectionNode1->SetSelectionList(ids1.Get());
    ...
    selectionNode2->SetSelectionList(ids2.Get());

    vtkNew<vtkSelection> selection1;
    selection1->AddNode(selectionNode1.Get());

    vtkNew<vtkSelection> selection2;
    selection2->AddNode(selectionNode2.Get());

    vtkNew<vtkSelection> selectionCombined;
    selectionCombined->AddNode(selectionNode1.Get());
    selectionCombined->Union(selectionNode2.Get());
    ...
    extractSelection1->SetInputData(1, selection1.Get());
    ...
    extractSelection2->SetInputData(1, selection2.Get());
    ...
    extractSelectionCombined->SetInputData(1, selectionCombined.Get());
    ...
    */
    ...
    originalActor->SetMapper(mapperOriginal.Get());
    ...
    actorCombined->SetMapper(mapperCombined.Get());
    ...
    originalRenderer->SetActiveCamera(camera.Get());
    ...
    selection1Renderer->SetActiveCamera(camera.Get());
    ...
    selection2Renderer->SetActiveCamera(camera.Get());

    vtkNew<vtkRenderer> selectionCombinedRenderer;
    selectionCombinedRenderer->SetViewport(selectionCombinedViewport);
    selectionCombinedRenderer->SetBackground(colors->GetColor3d("Ren3").GetData());
    selectionCombinedRenderer->SetActiveCamera(camera.Get());

    renderWindow->AddRenderer(originalRenderer.Get());
    renderWindow->AddRenderer(selection1Renderer.Get());
    renderWindow->AddRenderer(selection2Renderer.Get());
    renderWindow->AddRenderer(selectionCombinedRenderer.Get());

    originalRenderer->AddActor(originalActor.Get());
    selection1Renderer->AddActor(actor1.Get());
    selection2Renderer->AddActor(actor2.Get());
    selectionCombinedRenderer->AddActor(actorCombined.Get());

...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
        ...
        transformPD->SetInputData(polyData1.Get());
        transformPD->Update();
    ...
    select->SetInputData(polyData1.Get());
    ...
    int outsideId =
        threshold->AddBandpassIntervalSet(0, 0, vtkDataObject::FIELD_ASSOCIATION_POINTS, "SelectedPoints", 0, 1);
    // Inside points have a 1 value in ALL points of a cell
    int insideId =
        threshold->AddBandpassIntervalSet(1, 1, vtkDataObject::FIELD_ASSOCIATION_POINTS, "SelectedPoints", 0, 1);
    // Border points have a 0 or a 1 in at least one point of a cell
    int borderId = threshold->AddIntervalSet(0, 1, vtkMultiThreshold::OPEN, vtkMultiThreshold::OPEN,
    ...
    outsideMapper->SetInputData(dynamic_cast<vtkUnstructuredGrid *>(
        vtkMultiBlockDataSet::SafeDownCast(threshold->GetOutput()->GetBlock(outsideId))->GetBlock(0)));
    outsideMapper->ScalarVisibilityOff();

    vtkNew<vtkActor> outsideActor;
    outsideActor->SetMapper(outsideMapper.Get());
    ...
    insideMapper->SetInputData(dynamic_cast<vtkUnstructuredGrid *>(
        vtkMultiBlockDataSet::SafeDownCast(threshold->GetOutput()->GetBlock(insideId))->GetBlock(0)));
    insideMapper->ScalarVisibilityOff();

    vtkNew<vtkActor> insideActor;
    insideActor->SetMapper(insideMapper.Get());
    ...
    borderMapper->SetInputData(dynamic_cast<vtkUnstructuredGrid *>(
        vtkMultiBlockDataSet::SafeDownCast(threshold->GetOutput()->GetBlock(borderId))->GetBlock(0)));
    borderMapper->ScalarVisibilityOff();

    vtkNew<vtkActor> borderActor;
    borderActor->SetMapper(borderMapper.Get());
    ...
    surfaceMapper->SetInputData(polyData2.Get());
    ...
    renderer->AddActor(outsideActor.Get());
    renderer->AddActor(insideActor.Get());
    renderer->AddActor(borderActor.Get());

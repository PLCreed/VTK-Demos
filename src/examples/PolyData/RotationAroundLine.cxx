...
int main(int, char *[])
{
    ...
    actor1->SetMapper(coneMapper1.Get());

    // Actor for rotated arrow
    vtkNew<vtkActor> actor2;
    actor2->SetMapper(coneMapper2.Get());

    // Color the original arrow
    actor1->GetProperty()->SetColor(colors->GetColor3d("LightCoral").GetData());
    // Color rotated arrow
    actor2->GetProperty()->SetColor(colors->GetColor3d("PaleTurquoise").GetData());

    // Assign actor to the renderer
    ren->AddActor(actor1.Get());
    ren->AddActor(actor2.Get());

...
public:
    ...
    virtual void Execute(vtkObject *caller, unsigned long, void *)
    {
        ...
        this->Level =
            vtkMath::Round(static_cast<vtkSliderRepresentation *>(sliderWidget->GetRepresentation())->GetValue());
...
int main(int, char *[])
{
    ...
    pointsActor->SetMapper(pointsMapper.Get());
    pointsActor->GetProperty()->SetInterpolationToFlat();
    pointsActor->GetProperty()->SetColor(colors->GetColor3d("Seashell").GetData());
    ...
    pointLocator->GenerateRepresentation(0, polydata.Get());

    vtkNew<vtkPolyDataMapper> locatorTreeMapper;
    locatorTreeMapper->SetInputData(polydata.Get());

    vtkNew<vtkActor> locatorTreeActor;
    locatorTreeActor->SetMapper(locatorTreeMapper.Get());
    locatorTreeActor->GetProperty()->SetInterpolationToFlat();
    locatorTreeActor->GetProperty()->SetRepresentationToWireframe();
    locatorTreeActor->GetProperty()->SetColor(colors->GetColor3d("MistyRose").GetData());
    ...
    renderer->AddActor(pointsActor.Get());
    renderer->AddActor(locatorTreeActor.Get());
    ...
    SetSliderColors(sliderRep.Get());

    vtkNew<vtkSliderWidget> sliderWidget;
    sliderWidget->SetInteractor(renderWindowInteractor.Get());
    sliderWidget->SetRepresentation(sliderRep.Get());
    sliderWidget->SetAnimationModeToAnimate();
    sliderWidget->EnabledOn();

    vtkNew<vtkSliderCallback> callback;
    callback->PointLocator = pointLocator.Get();
    callback->PolyData = polydata.Get();
    callback->Renderer = renderer.Get();

    sliderWidget->AddObserver(vtkCommand::InteractionEvent, callback.Get());

...
int main(int argc, char *argv[])
{
    ...
    std::array<double, 3> point0, point1, point2, point3, point4, point5, point6, point7;
    ...
    lengths[0] = std::sqrt(vtkMath::Distance2BetweenPoints(centers[0].data(), endPoints[0].data())) / 2.0;
    ...
    lengths[1] = std::sqrt(vtkMath::Distance2BetweenPoints(centers[1].data(), endPoints[1].data())) / 2.0;
    ...
    lengths[2] = std::sqrt(vtkMath::Distance2BetweenPoints(centers[2].data(), endPoints[2].data())) / 2.0;
    ...
    // enclosedPoints->SetInputData(polyData.Get());
    ...
    repMapper->SetInputData(obbPolydata.Get());
    vtkNew<vtkActor> repActor;
    repActor->SetMapper(repMapper.Get());
    ...
    for (auto i = 0; i < 3; ++i) {
        ...
        pa1->SetMapper(pm1.Get());
        renderer->AddActor(pa1.Get());
        ...
        pa2->SetMapper(pm2.Get());
        renderer->AddActor(pa2.Get());
    }
    ...
    renderer->AddActor(originalActor.Get());
    renderer->AddActor(cylinderActor.Get());
    // renderer->AddActor(repActor.Get());
    ...
    //     for (double r = radius; enclosedPoints->GetOutput(1)->GetPoints()->GetNumberOfPoints() > 4; r +=
    //     adjustedIncr) {
    //         tube->SetRadius(r);
    //         tube->Update();
    //         clean->Update();
    ...
    //             std::cout << " excluded points: " << enclosedPoints->GetOutput(1)->GetPoints()->GetNumberOfPoints()
    //                       << std::endl;
    //             renderWindow->Render();
    //         } else {
    //             break;
...
double MakeAQuad(std::vector<std::array<double, 3>> &points, std::array<double, 3> &center)
{
    vtkNew<vtkQuad> aQuad;

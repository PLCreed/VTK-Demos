...
int main(int, char *[])
{
    ...
    // Should be inside
    cellId = cellTree->FindCell(testInside, 0, cell.Get(), pcoords, weights);
    if (cellId >= 0) {
    ...
    // Should be outside
    cellId = cellTree->FindCell(testOutside, 0, cell.Get(), pcoords, weights);
    if (cellId >= 0) {
        std::cout << "ERROR: Found point in cell " << cellId << " but it should be outside the domain." << std::endl;
        returnValue = EXIT_FAILURE;
    } else {

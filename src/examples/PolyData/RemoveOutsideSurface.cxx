...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Center of data: " << center[0] << ", " << center[1] << ", " << center[2] << std::endl;
    polyData->GetPoints()->GetBounds(bounds);
    std::cout << "Bounds of data: " << bounds[0] << ", " << bounds[1] << ", " << bounds[2] << ", " << bounds[3] << ", "
              << bounds[4] << ", " << bounds[5] << std::endl;

    // Extract all regions and label cells with region number
    vtkNew<vtkConnectivityFilter> connectivityFilter;
    connectivityFilter->SetInputData(polyData.Get());
    ...
    vtkIdTypeArray *cd = dynamic_cast<vtkIdTypeArray *>(connectivityFilter->GetOutput()->GetCellData()->GetScalars());
    ...
    for (it->InitTraversal(); !it->IsDoneWithTraversal(); it->GoToNextCell(), ++originalCellId) {
    ...
    renderer->AddActor(extractedActor.Get());

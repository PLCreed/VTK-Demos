...
int main(int, char *[])
{
    ...
    torusTF->SetTransform(torusT.Get());
    ...
    rhFnSrc->SetParametricFunction(rh.Get());
    rhFnSrc->Update();
    ...
    for (auto idx = 0; idx < static_cast<int>(sources.size()); ++idx) {
        ...
        auto scalarRange = sources[idx]->GetPointData()->GetScalars(curvatures[idx].c_str())->GetRange();
        ...
        mappers[idx]->SetLookupTable(lut.Get());

        actors[idx]->SetMapper(mappers[idx]);

        textmappers[idx]->SetInput(curvatureName.c_str());
        textmappers[idx]->SetTextProperty(textProperty.Get());
    ...
    for (auto row = 0; row < gridDimensions; ++row) {
        for (auto col = 0; col < gridDimensions; ++col) {
            ...
            renderers[idx]->SetViewport(static_cast<double>(col) / gridDimensions,
...
void AdjustEdgeCurvatures(vtkPolyData *source, std::string const &curvatureName, double const &epsilon)
{
    ...
    // idFilter->SetPointIdsArrayName(name);
    // idFilter->SetCellIdsArrayName(name);
    idFilter->Update();
...
vtkSmartPointer<vtkLookupTable> GetDivergingLut1()
{
    ...
    vtkNew<vtkColorTransferFunction> ctf;
    ctf->SetColorSpaceToDiverging();
    ctf->AddRGBPoint(0.0, colors->GetColor3d("MidnightBlue").GetRed(), colors->GetColor3d("MidnightBlue").GetGreen(),
                     colors->GetColor3d("MidnightBlue").GetBlue());
    ctf->AddRGBPoint(0.5, colors->GetColor3d("Gainsboro").GetRed(), colors->GetColor3d("Gainsboro").GetGreen(),
                     colors->GetColor3d("Gainsboro").GetBlue());
    ctf->AddRGBPoint(1.0, colors->GetColor3d("DarkOrange").GetRed(), colors->GetColor3d("DarkOrange").GetGreen(),

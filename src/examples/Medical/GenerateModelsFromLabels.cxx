...
{
    if (argc < 4) {
        std::cout << "Usage: " << argv[0] << " InputVolume StartLabel EndLabel  e.g. Frog/frogtissue.mhd 1 29"
                  << std::endl;
        return EXIT_FAILURE;
    }
...
#ifdef USE_FLYING_EDGES
    selector->SetInputArrayToProcess(0, 0, 0, vtkDataObject::FIELD_ASSOCIATION_POINTS, vtkDataSetAttributes::SCALARS);
#else
    ...
    for (unsigned int i = startLabel; i <= endLabel; i++) {
        ...
        selector->ThresholdByLower(i);
        selector->ThresholdByUpper(i);

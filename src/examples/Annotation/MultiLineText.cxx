...
int main(int, char *[])
{
    ...
    multiLineTextProp->ShallowCopy(singleLineTextProp.Get());
    ...
    singleLineTextActorC->SetMapper(singleLineTextC.Get());
    ...
    singleLineTextActorT->SetMapper(singleLineTextT.Get());
    ...
    tprop->ShallowCopy(multiLineTextProp.Get());
    tprop->SetJustificationToLeft();
    tprop->SetVerticalJustificationToTop();
    tprop->SetColor(colors->GetColor3d("Tomato").GetData());

    vtkNew<vtkActor2D> textActorL;
    textActorL->SetMapper(textMapperL.Get());
    ...
    textActorC->SetMapper(textMapperC.Get());
    ...
    textActorR->SetMapper(textMapperR.Get());
    ...
    mapper->SetInputData(Grid.Get());
    mapper->SetTransformCoordinate(normCoords.Get());
    vtkNew<vtkActor2D> gridActor;
    gridActor->SetMapper(mapper.Get());
    ...
    renderer->AddActor2D(textActorL.Get());
    renderer->AddActor2D(textActorC.Get());
    renderer->AddActor2D(textActorR.Get());
    renderer->AddActor2D(singleLineTextActorB.Get());
    renderer->AddActor2D(singleLineTextActorC.Get());
    renderer->AddActor2D(singleLineTextActorT.Get());
    renderer->AddActor2D(gridActor.Get());

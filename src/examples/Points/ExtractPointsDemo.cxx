...
int main(int /*argc*/, char * /* argv */[])
{
    ...
    functions.push_back(sphere.Get());
    functions.push_back(cone.Get());
    functions.push_back(cylinder.Get());
    functions.push_back(superquadric.Get());
    ...
    renderWindow->SetSize(rendererSize * gridXDimensions, rendererSize * gridYDimensions);
    ...
    for (int row = 0; row < static_cast<int>(gridYDimensions); row++) {
        for (int col = 0; col < static_cast<int>(gridXDimensions); col++) {
            ...
            glyphActor->SetMapper(glyph.Get());
            glyphActor->GetProperty()->SetColor(colors->GetColor3d("MistyRose").GetData());

            renderers[index]->AddActor(glyphActor.Get());

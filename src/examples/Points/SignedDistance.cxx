...
int main(int argc, char *argv[])
{
    ...
    normals->SetInputData(polyData.Get());
    ...
    distance->SetBounds(bounds[0] - range[0] * 0.1, bounds[1] + range[0] * 0.1, bounds[2] - range[1] * 0.1,
                        bounds[3] + range[1] * 0.1, bounds[4] - range[2] * 0.1, bounds[5] + range[2] * 0.1);
    ...
    sagittalColors->SetLookupTable(hueLut.Get());
    ...
    axialColors->SetLookupTable(hueLut.Get());
    axialColors->Update();
    ...
    coronalColors->SetLookupTable(hueLut.Get());
    ...
    //
    ren1->AddActor(sagittal.Get());
    ren1->AddActor(axial.Get());
    ren1->AddActor(coronal.Get());
    ren1->AddActor2D(scalarBar.Get());
    ...
    iren->Initialize();
    iren->Start();
    std::cout << distance->GetOutput()->GetScalarRange()[0] << ", " << distance->GetOutput()->GetScalarRange()[1]
              << std::endl;

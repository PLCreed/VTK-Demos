...
int main(int, char *[])
{
    ...
    perspectiveTransform->SetMatrix(m.Get());

    vtkNew<vtkTransform> transform;
    transform->SetMatrix(m.Get());
    ...
    std::cout << "Standard projection: " << normalProjection[0] << " " << normalProjection[1] << " "
              << normalProjection[2] << std::endl;

    double perspectiveProjection[3];
    perspectiveTransform->TransformPoint(p, perspectiveProjection);
    std::cout << "Perspective projection: " << perspectiveProjection[0] << " " << perspectiveProjection[1] << " "

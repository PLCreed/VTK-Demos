...
int main(int, char *[])
{
    ...
    auto outline_pd = as_polyline(points.Get(), LEVEL);
    ...
    triangles->InsertNextCell(t.Get());
    data->InsertNextValue(0);

    as_triangles(0, stride, triangles.Get(), 1, data.Get());
    as_triangles(stride, 2 * stride, triangles.Get(), 1, data.Get());
    as_triangles(2 * stride, 3 * stride, triangles.Get(), 1, data.Get());

    vtkNew<vtkPolyData> triangle_pd;
    triangle_pd->SetPoints(outline_pd->GetPoints());
    triangle_pd->SetPolys(triangles.Get());
    triangle_pd->GetCellData()->SetScalars(data.Get());
    ...
    triangle_mapper->SetInputData(triangle_pd.Get());
    triangle_mapper->SetScalarRange(0.0, LEVEL);
    triangle_mapper->SetLookupTable(lut.Get());

    vtkNew<vtkActor> outline_actor;
    outline_actor->SetMapper(outline_mapper.Get());

    vtkNew<vtkActor> triangle_actor;
    triangle_actor->SetMapper(triangle_mapper.Get());

    vtkNew<vtkRenderer> outline_ren;
    outline_ren->AddActor(outline_actor.Get());
    ...
    triangle_ren->AddActor(triangle_actor.Get());
    ...
    renw->AddRenderer(outline_ren.Get());
    renw->AddRenderer(triangle_ren.Get());
    renw->SetSize(800, 400);
    renw->SetWindowName("KochSnowflake");

    vtkNew<vtkRenderWindowInteractor> iren;
    iren->SetRenderWindow(renw.Get());
...
void as_triangles(int start, int end, vtkCellArray *cells, int level, vtkIntArray *data)
{
    if (end - start >= 3) {
        ...
        cells->InsertNextCell(triangle.Get());

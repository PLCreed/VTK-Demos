...
int main(int, char *[])
{
    ...
    ImageData(imageData.Get(), gridSize);
    vtkNew<vtkDataSetMapper> imageDataMapper;
    imageDataMapper->SetInputData(imageData.Get());
    imageDataMapper->ScalarVisibilityOff();

    vtkNew<vtkActor> imageDataActor;
    imageDataActor->GetProperty()->SetRepresentationToWireframe();
    imageDataActor->SetMapper(imageDataMapper.Get());
    imageDataActor->GetProperty()->SetColor(colors->GetColor3d("MidnightBlue").GetData());

    vtkNew<vtkRectilinearGrid> rectilinearGrid;
    RectilinearGrid(rectilinearGrid.Get(), gridSize);
    vtkNew<vtkDataSetMapper> rectilinearGridMapper;
    rectilinearGridMapper->SetInputData(rectilinearGrid.Get());

    vtkNew<vtkActor> rectilinearGridActor;
    rectilinearGridActor->GetProperty()->SetRepresentationToWireframe();
    rectilinearGridActor->SetMapper(rectilinearGridMapper.Get());
    rectilinearGridActor->GetProperty()->SetColor(colors->GetColor3d("MidnightBlue").GetData());

    vtkNew<vtkStructuredGrid> structuredGrid;
    StructuredGrid(structuredGrid.Get(), gridSize);
    vtkNew<vtkDataSetMapper> structuredGridMapper;
    structuredGridMapper->SetInputData(structuredGrid.Get());

    vtkNew<vtkActor> structuredGridActor;
    structuredGridActor->GetProperty()->SetRepresentationToWireframe();
    structuredGridActor->SetMapper(structuredGridMapper.Get());
    structuredGridActor->GetProperty()->SetColor(colors->GetColor3d("MidnightBlue").GetData());
    ...
    // right
    leftRenderer->AddActor(imageDataActor.Get());
    centerRenderer->AddActor(rectilinearGridActor.Get());
    rightRenderer->AddActor(structuredGridActor.Get());
...
void RectilinearGrid(vtkRectilinearGrid *data, const int gridSize)
{
    ...
    data->SetXCoordinates(xCoords.Get());
    data->SetYCoordinates(yCoords.Get());
    data->SetZCoordinates(zCoords.Get());

...
public:
    ...
    virtual void Execute(vtkObject *caller, unsigned long, void *)
    {
        ...
        text << "cursor: " << std::fixed << std::setprecision(4) << position[0] << ", " << position[1] << ", "
             << position[2];
...
int main(int argc, char *argv[])
{
    ...
    mapper->SetInputData(inputPolyData.Get());
    ...
    myCallback->PolyData = point.Get();
    myCallback->CursorActor = glyphActor.Get();
    myCallback->PositionActor = textActor.Get();

    // The point widget is used probe the dataset.
    //
    vtkNew<vtkPointWidget> pointWidget;
    pointWidget->SetInteractor(iren.Get());
    pointWidget->SetInputData(inputPolyData.Get());
    pointWidget->AllOff();
    pointWidget->PlaceWidget();
    pointWidget->AddObserver(vtkCommand::InteractionEvent, myCallback.Get());

    ren1->AddActor(glyphActor.Get());
    ren1->AddActor(actor.Get());
    ren1->AddActor2D(textActor.Get());

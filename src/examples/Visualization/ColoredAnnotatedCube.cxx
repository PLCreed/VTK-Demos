...
int main(int, char *[])
{
    ...
    coneMapper->SetLookupTable(lut.Get());
    ...
    contourLineActor->SetMapper(contourLineMapper.Get());
    contourLineActor->GetProperty()->SetColor(colors->GetColor3d("DimGray").GetData());

    // Set up the Orientation Marker Widget.
    auto prop_assembly = MakeAnnotatedCubeActor(colors.Get());
    vtkNew<vtkOrientationMarkerWidget> om1;
    om1->SetOrientationMarker(prop_assembly);
    om1->SetInteractor(iRen.Get());
    om1->SetDefaultRenderer(ren.Get());
    om1->On();
    om1->InteractiveOn();
    ...
    om2->SetOrientationMarker(axes.Get());
    ...
    ren->AddActor(coneActor.Get());
    ren->AddActor(contourLineActor.Get());
...
vtkSmartPointer<vtkPropAssembly> MakeAnnotatedCubeActor(vtkNamedColors *colors)
{
    ...
    annotated_cube->GetTextEdgesProperty()->SetColor(colors->GetColor3d("Black").GetData());
    annotated_cube->GetTextEdgesProperty()->SetLineWidth(1);

    annotated_cube->GetXPlusFaceProperty()->SetColor(colors->GetColor3d("Turquoise").GetData());
    annotated_cube->GetXMinusFaceProperty()->SetColor(colors->GetColor3d("Turquoise").GetData());
    annotated_cube->GetYPlusFaceProperty()->SetColor(colors->GetColor3d("Mint").GetData());
    annotated_cube->GetYMinusFaceProperty()->SetColor(colors->GetColor3d("Mint").GetData());
    annotated_cube->GetZPlusFaceProperty()->SetColor(colors->GetColor3d("Tomato").GetData());
    annotated_cube->GetZMinusFaceProperty()->SetColor(colors->GetColor3d("Tomato").GetData());
    ...
    cube_source->GetOutput()->GetCellData()->SetScalars(face_colors.Get());
    cube_source->Update();
    ...
    cube_actor->SetMapper(cube_mapper.Get());

    // Assemble the colored cube and annotated cube texts into a composite prop.
    vtkSmartPointer<vtkPropAssembly> prop_assembly = vtkSmartPointer<vtkPropAssembly>::New();
    prop_assembly->AddPart(annotated_cube.Get());
    prop_assembly->AddPart(cube_actor.Get());

...
int main(int /* argc */, char * /* argv */[])
{
    ...
    selectRect->SetPoints(pts.Get());
    selectRect->SetLines(rect.Get());

    vtkNew<vtkPolyDataMapper2D> rectMapper;
    rectMapper->SetInputData(selectRect.Get());

    vtkNew<vtkActor2D> rectActor;
    rectActor->SetMapper(rectMapper.Get());
    ...
    sphereActor->GetProperty()->SetColor(colors->GetColor3d("BurlyWood").GetData());
    ...
    cellMapper->GetLabelTextProperty()->SetColor(colors->GetColor3d("DarkGreen").GetData());

    vtkNew<vtkActor2D> cellLabels;
    cellLabels->SetMapper(cellMapper.Get());
    ...
    ren1->AddActor2D(rectActor.Get());
    ren1->AddActor2D(pointLabels.Get());
    ren1->AddActor2D(cellLabels.Get());
    ...
    VTKData data = {visPts.Get(), visCells.Get(), pts.Get(), renWin.Get()};

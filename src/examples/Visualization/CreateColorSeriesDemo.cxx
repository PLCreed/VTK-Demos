...
int main(int argc, char *argv[])
{
    ...
    aPlane->GetOutput()->GetCellData()->SetScalars(cellData.Get());

    vtkNew<vtkNamedColors> colors;

    vtkNew<vtkLookupTable> lut;
    if (seriesName == "Blue") {
        CreateLookupTableVTKBlue(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "Brown") {
        CreateLookupTableVTKBrown(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "Red") {
        CreateLookupTableVTKRed(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "Orange") {
        CreateLookupTableVTKOrange(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "White") {
        CreateLookupTableVTKWhite(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "Grey") {
        CreateLookupTableVTKGrey(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "Magenta") {
        CreateLookupTableVTKMagenta(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "Cyan") {
        CreateLookupTableVTKCyan(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "Yellow") {
        CreateLookupTableVTKYellow(lut.Get(), resolution * resolution + 1);
    } else if (seriesName == "Green") {
        CreateLookupTableVTKGreen(lut.Get(), resolution * resolution + 1);
    } else {
...
void CreateLookupTableVTKBlue(vtkLookupTable *lut, int size)
{
    ...
    for (int i = 0; i < lut->GetNumberOfTableValues(); ++i) {
        ...
        lut->SetTableValue(i, color.GetRed() / 255., color.GetGreen() / 255., color.GetBlue() / 255., 1.);
    }
}

...
int main(int, char *[])
{
    ...
    image->GetPointData()->SetActiveVectors(image->GetPointData()->GetScalars()->GetName());
    ...
    glyphFilter->SetInputData(image.Get());
    glyphFilter->Update();

    // Create actors
    vtkNew<vtkImageSliceMapper> imageMapper;
    imageMapper->SetInputData(image.Get());

    vtkNew<vtkImageSlice> imageSlice;
    imageSlice->SetMapper(imageMapper.Get());
    ...
    vectorActor->SetMapper(vectorMapper.Get());

    // Setup renderer
    vtkNew<vtkRenderer> renderer;
    renderer->AddViewProp(imageSlice.Get());
    renderer->AddViewProp(vectorActor.Get());

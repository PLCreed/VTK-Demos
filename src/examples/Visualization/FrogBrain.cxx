...
{
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << "InputFile(.mhd) Tissue e.g.frogtissue.mhd brain" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    renderWindow->AddRenderer(rendererLeft.Get());
    renderWindow->AddRenderer(rendererRight.Get());
    vtkNew<vtkRenderWindowInteractor> renderWindowInteractor;
    renderWindowInteractor->SetRenderWindow(renderWindow.Get());
    vtkNew<vtkActor> actor;
    actor->GetProperty()->SetDiffuseColor(colorLut->GetTableValue(tissueMap[argv[2]]));
    CreateFrogActor(argv[1], tissueMap[argv[2]], actor.Get());
    rendererLeft->AddActor(actor.Get());

    vtkNew<vtkActor> actorSmooth;
    actorSmooth->GetProperty()->SetDiffuseColor(colorLut->GetTableValue(tissueMap[argv[2]]));
    actorSmooth->GetProperty()->SetDiffuse(1.0);
    actorSmooth->GetProperty()->SetSpecular(.5);
    actorSmooth->GetProperty()->SetSpecularPower(100);
    CreateSmoothFrogActor(argv[1], tissueMap[argv[2]], actorSmooth.Get());
    rendererRight->AddActor(actorSmooth.Get());

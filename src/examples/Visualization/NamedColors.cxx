...
std::vector<std::string> FindSynonyms(const std::string &color, vtkNamedColors *namedColors);
...
int main(int, char *[])
{
    ...
    // synonyms.
    PrintColors(namedColors.Get());
    PrintSynonyms(namedColors.Get());
    ...
    auto matchingColors = FindSynonyms("My Red", namedColors.Get());
    ...
    renderer->AddActor(contourLineActor.Get());
...
std::vector<std::string> FindSynonyms(const std::string &color, vtkNamedColors *namedColors)
{
    ...
    std::string lcColor;
    std::transform(color.begin(), color.end(), std::back_inserter(lcColor), (int (*)(int))std::tolower);
...
void PrintColors(vtkNamedColors *namedColors)
{
    ...
    auto max_str = std::max_element(colors.begin(), colors.end(),
...
void PrintSynonyms(vtkNamedColors *namedColors)
{
    ...
    for (auto const &p : synonyms) {
        auto max_str = std::max_element(p.begin(), p.end(),
                                        [](std::string const &a, std::string const &b)
                                        {
                                            return a.size() < b.size();
                                        });
        max_str_len = (max_str_len < max_str->size()) ? max_str->size() : max_str_len;
    }
    for (auto const &p : synonyms) {
        size_t n = 0;
        for (auto const &q : p) {

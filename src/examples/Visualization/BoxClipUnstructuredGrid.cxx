...
int main(int argc, char *argv[])
{
    ...
    boxClip->SetBoxClip(minusx, minBoxPoint, minusy, minBoxPoint, minusz, minBoxPoint, plusx, maxBoxPoint, plusy,
                        maxBoxPoint, plusz, maxBoxPoint);
    ...
    mapperIn->SetLookupTable(lut1.Get());
    mapperIn->SetColorModeToMapScalars();

    vtkNew<vtkActor> actorIn;
    actorIn->SetMapper(mapperIn.Get());
    ...
    mapperOut->SetLookupTable(lut1.Get());
    mapperOut->SetColorModeToMapScalars();

    vtkNew<vtkActor> actorOut;
    actorOut->SetMapper(mapperOut.Get());
    actorOut->AddPosition(-0.5 * (maxBoxPoint[0] - minBoxPoint[0]), -0.5 * (maxBoxPoint[1] - minBoxPoint[1]),
    ...
    renderer->AddActor(actorIn.Get());
    renderer->AddActor(actorOut.Get());

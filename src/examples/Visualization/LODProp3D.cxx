...
namespace
{
void RefreshCallback(vtkObject *vtkNotUsed(caller), long unsigned int vtkNotUsed(eventId), void *clientData,
...
int main(int, char *[])
{
    ...
    propertyLowRes->SetDiffuseColor(colors->GetColor3d("BlanchedAlmond").GetData());
    ...
    prop->AddLOD(lowResMapper.Get(), propertyLowRes.Get(), 0.0);
    prop->AddLOD(highResMapper.Get(), propertyHighRes.Get(), 0.0);
    ...
    prop->SetAllocatedRenderTime(1e-12, renderer.Get());
    ...
    renderer->AddActor(prop.Get());
    renderer->SetBackground(colors->GetColor3d("SlateGray").GetData());

    vtkNew<vtkCallbackCommand> refreshCallback;
    refreshCallback->SetCallback(RefreshCallback);
    refreshCallback->SetClientData(prop.Get());

    renderWindow->AddObserver(vtkCommand::ModifiedEvent, refreshCallback.Get());

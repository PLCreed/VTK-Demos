 ...
 * @return
 */
void AdjustEdgeCurvatures(vtkPolyData *source, std::string const &curvatureName, double const &epsilon = 1.0e-08);
 ...
 * @return
 */
void ConstrainCurvatures(vtkPolyData *source, std::string const &curvatureName, double const &lowerBound = 0.0,
...
@return The glyphs.
*/
vtkSmartPointer<vtkGlyph3D> GetGlyphs(vtkPolyData *src, double const &scaleFactor = 1.0,
                                      bool const &reverseNormals = false);

std::map<int, std::vector<double>> GetBands(double const dR[2], int const &numberOfBands, int const &precision = 2,
...
band.
*/
std::map<int, std::vector<double>> GetCustomBands(double const dR[2], int const &numberOfBands,
 ...
 */
std::map<int, int> GetFrequencies(std::map<int, std::vector<double>> &bands, vtkPolyData *src);
//!
/*
 ...
 */
void AdjustFrequencyRanges(std::map<int, std::vector<double>> &bands, std::map<int, int> &freq);

void PrintBandsFrequencies(std::map<int, std::vector<double>> const &bands, std::map<int, int> &freq,
...
int main(int, char *[])
{
    ...
    std::string curvature = (gaussianCurvature) ? "Gauss_Curvature" : "Mean_Curvature";

    vtkNew<vtkCurvatures> cc;
    cc->SetInputData(source.Get());
    std::vector<std::string> needsAdjusting {"Hills", "ParametricTorus", "Plane", "RandomHills", "Torus"};
    if (gaussianCurvature) {
        cc->SetCurvatureTypeToGaussian();
        cc->Update();
        if (std::find(needsAdjusting.begin(), needsAdjusting.end(), desiredSurface) != needsAdjusting.end()) {
    ...
    auto scalarRangeCurvatures = cc->GetOutput()->GetPointData()->GetScalars(curvature.c_str())->GetRange();
    auto scalarRangeElevation = cc->GetOutput()->GetPointData()->GetScalars("Elevation")->GetRange();
    ...
    for (std::map<int, std::vector<double>>::const_iterator p = bands.begin(); p != bands.end(); ++p) {
    ...
    colors->SetColor("BkgColor", std::array<unsigned char, 4> {179, 204, 255, 255}.data());
    ...
    srcMapper->SetLookupTable(lut.Get());
    srcMapper->SetScalarModeToUseCellData();

    vtkNew<vtkActor> srcActor;
    srcActor->SetMapper(srcMapper.Get());
    ...
    // Colour by scalars.
    glyphMapper->SetLookupTable(lut1.Get());
    ...
    scalarBar->SetLookupTable(lutr.Get());
    scalarBar->SetTitle(curvatureType.c_str());
    scalarBar->GetTitleTextProperty()->SetColor(colors->GetColor3d("AliceBlue").GetData());
    scalarBar->GetLabelTextProperty()->SetColor(colors->GetColor3d("AliceBlue").GetData());
    scalarBar->GetAnnotationTextProperty()->SetColor(colors->GetColor3d("AliceBlue").GetData());
    ...
    scalarBarElev->SetLookupTable(lut1.Get());
    scalarBarElev->SetTitle("Elevation");
    scalarBarElev->GetTitleTextProperty()->SetColor(colors->GetColor3d("AliceBlue").GetData());
    scalarBarElev->GetLabelTextProperty()->SetColor(colors->GetColor3d("AliceBlue").GetData());
    scalarBarElev->GetAnnotationTextProperty()->SetColor(colors->GetColor3d("AliceBlue").GetData());
    ...
    // add actors
    ren->AddViewProp(srcActor.Get());
    ren->AddViewProp(edgeActor.Get());
    ren->AddViewProp(glyphActor.Get());
    ren->AddActor2D(scalarBar.Get());
    ren->AddActor2D(scalarBarElev.Get());
...
void AdjustEdgeCurvatures(vtkPolyData *source, std::string const &curvatureName, double const &epsilon)
{
    auto PointNeighbourhood = [&source](vtkIdType const &pId) -> std::set<vtkIdType>
    {
        // Extract the topological neighbors for point pId. In two steps:
        //  1) source->GetPointCells(pId, cellIds)
        //  2) source->GetCellPoints(cellId, cellPointIds) for all cellId in cellIds
        vtkNew<vtkIdList> cellIds;
        source->GetPointCells(pId, cellIds.Get());
        std::set<vtkIdType> neighbours;
        for (vtkIdType i = 0; i < cellIds->GetNumberOfIds(); ++i) {
            auto cellId = cellIds->GetId(i);
            vtkNew<vtkIdList> cellPointIds;
            source->GetCellPoints(cellId, cellPointIds.Get());
    ...
    auto ComputeDistance = [&source](vtkIdType const &ptIdA, vtkIdType const &ptIdB)
    {
        ...
        std::transform(std::begin(ptA), std::end(ptA), std::begin(ptB), std::begin(ptC), std::minus<double>());
        // Calculate the norm.
        auto result = std::sqrt(std::inner_product(std::begin(ptC), std::end(ptC), std::begin(ptC), 0.0));
        return result;
    };
    ...
    idFilter->SetIdsArrayName(name.c_str());
    idFilter->Update();
    ...
    auto edgeAarray = edges->GetOutput()->GetPointData()->GetAbstractArray(name.c_str());
    ...
    for (auto const pId : boundaryIds) {
        ...
        std::set_difference(pIdsNeighbors.begin(), pIdsNeighbors.end(), pIdsSet.begin(), pIdsSet.end(),
        ...
        if (curvsNonZero.size() > 0) {
            ...
            newCurv = std::inner_product(curvsNonZero.begin(), curvsNonZero.end(), weights.begin(), 0.0);
        } else {
            // Corner case.
            // countInvalid += 1;
    ...
    source->GetPointData()->AddArray(adjustedCurvatures.Get());
    source->GetPointData()->SetActiveScalars(curvatureName.c_str());
}

void ConstrainCurvatures(vtkPolyData *source, std::string const &curvatureName, double const &lowerBound,
...
vtkSmartPointer<vtkPolyData> GetHills()
{
    ...
    plane->SetPoints(points.Get());

    // Triangulate the grid.
    vtkNew<vtkDelaunay2D> delaunay;
    delaunay->SetInputData(plane.Get());
    delaunay->Update();
    ...
    std::vector<std::array<double, 5>> hd {
        {-2.5, -2.5, 2.5, 6.5, 3.5},
        {2.5,  2.5,  2.5, 2.5, 2  },
        {5.0,  -2.5, 1.5, 1.5, 2.5},
        {-5.0, 5,    2.5, 3.0, 3  }
    ...
    polydata->GetPointData()->SetScalars(elevation.Get());
    polydata->GetPointData()->GetScalars()->SetName("Elevation");
    polydata->GetPointData()->SetTCoords(textures.Get());
...
vtkSmartPointer<vtkPolyData> GetParametricHills()
{
    ...
    source->SetParametricFunction(fn.Get());
...
vtkSmartPointer<vtkLookupTable> GetOrdinaLUT()
{
    ...
    // Make the lookup table.
    vtkSmartPointer<vtkLookupTable> lut = vtkSmartPointer<vtkLookupTable>::New();
    colorSeries->BuildLookupTable(lut.Get(), vtkColorSeries::ORDINAL);
    lut->SetNanColor(0, 0, 0, 1);
...
vtkSmartPointer<vtkLookupTable> ReverseLUT(vtkLookupTable *lut)
{
    ...
    vtkSmartPointer<vtkLookupTable> lutr = vtkSmartPointer<vtkLookupTable>::New();
    lutr->DeepCopy(lut);
...
vtkSmartPointer<vtkGlyph3D> GetGlyphs(vtkPolyData *src, double const &scaleFactor, bool const &reverseNormals)
{
...
std::map<int, std::vector<double>> GetBands(double const dR[2], int const &numberOfBands, int const &precision,
                           ...
                           int const &precision)
{
    ...
    for (std::map<int, std::vector<double>>::const_iterator p = bands.begin(); p != bands.end(); ++p) {
        total += freq[p->first];
        for (std::vector<double>::const_iterator q = p->second.begin(); q != p->second.end(); ++q) {
            if (q == p->second.begin()) {
                os << std::setw(4) << idx << " [";
            }
            if (q == std::prev(p->second.end())) {
                os << std::fixed << std::setw(width) << std::setprecision(prec) << *q << "]: " << std::setw(8)
                   << freq[p->first] << "\n";
            } else {
    ...
    width = 3 * width + 13;
    os << std::left << std::setw(width) << "Total" << std::right << std::setw(8) << total << std::endl;

...
public:
    ...
    static void ParametricTorusSource(vtkPolyData *src)
    {
        ...
        torusSource->SetParametricFunction(torus.Get());
    ...
    static void EllipsoidSource(vtkPolyData *src)
    {
        ...
        ellipsoidSource->SetParametricFunction(ellipsoid.Get());
    ...
    static void BoySource(vtkPolyData *src)
    {
        ...
        boySource->SetParametricFunction(boy.Get());
    ...
    static void MobiusSource(vtkPolyData *src)
    {
        ...
        mobiusSource->SetParametricFunction(mobius.Get());
    ...
    static void ParametricRandomHills(vtkPolyData *src)
    {
        ...
        randomHillsSource->SetParametricFunction(randomHills.Get());
...
vtkSmartPointer<vtkActor> GlyphActor(vtkPolyData *source, int glyphPoints, double *scalarRange, double scaleFactor,
...
int main(int argc, char *argv[])
{
    ...
    case Sources::PARAMETRIC_TORUS :
        Sources::ParametricTorusSource(polyData.Get());
        break;
    case Sources::PARAMETRIC_ELLIPSOID :
        Sources::EllipsoidSource(polyData.Get());
        break;
    case Sources::BOY :
        Sources::BoySource(polyData.GetPointer());
        break;
    case Sources::SPHERE :
        Sources::SphereSource(polyData.Get());
        break;
    case Sources::MOBIUS :
        Sources::MobiusSource(polyData.Get());
        break;
    case Sources::CONE :
        Sources::ConeSource(polyData.Get());
        break;
    case Sources::RANDOM_HILLS :
        Sources::ParametricRandomHills(polyData.Get());
        break;
    case Sources::SUPERQUADRIC :
        Sources::SuperquadricSource(polyData.Get());
        break;
    default :
    ...
    actors.push_back(MakeSurfaceActor(polyData.Get(), scalarRange, lut));
    ...
    if (displayNormals) {
        ...
        glyphActors.push_back(GlyphActor(polyData.Get(), glyphPoints, scalarRange, scaleFactor, lut));
    }
    ...
    for (unsigned int i = 0; i < ren.size(); ++i) {
        ...
        ren[i]->SetActiveCamera(camera.Get());
        if (displayNormals) {
            ren[i]->AddActor(glyphActors[i]);
        }

        om.push_back(MakeOrientationMarker(ren[i], iren.Get()));
    }
                                     ...
                                     vtkColorTransferFunction *lut)
{
    ...
    glyphActor->SetMapper(arrowGlyphMapper.Get());
    return glyphActor;
}
...
vtkSmartPointer<vtkActor2D> MakeLabel(const char *textLabel, int renWinSize)
{
    ...
    mapper->SetTextProperty(textProperty.Get());
                                                                  ...
                                                                  vtkRenderWindowInteractor *iren)
{
    ...
    om->SetOrientationMarker(axesActor.Get());

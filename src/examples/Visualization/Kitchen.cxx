...
int main(int argc, char *argv[])
{
    ...
    std::array<unsigned char, 4> furnColor {
        {204, 204, 153, 255}
    };
    ...
    if (reader->GetOutput()->GetPointData()->GetVectors()) {
        auto maxVelocity = reader->GetOutput()->GetPointData()->GetVectors()->GetMaxNorm();
    ...
    outline->SetMapper(outlineMapper.Get());
    ...
    window1->SetMapper(mapWindow1.Get());
    ...
    window2->SetMapper(mapWindow2.Get());
    ...
    klower1->SetMapper(mapKlower1.Get());
    ...
    klower2->SetMapper(mapKlower2.Get());
    ...
    klower3->SetMapper(mapKlower3.Get());
    ...
    klower4->SetMapper(mapKlower4.Get());
    ...
    klower5->SetMapper(mapKlower5.Get());
    ...
    klower6->SetMapper(mapKlower6.Get());
    ...
    klower7->SetMapper(mapKlower7.Get());
    ...
    cookingPlate->SetMapper(mapCookingPlate.Get());
    ...
    filter->SetMapper(mapFilter.Get());
    ...
    lines->SetMapper(streamersMapper.Get());
    lines->GetProperty()->SetColor(colors->GetColor3d("Black").GetData());

    aren->TwoSidedLightingOn();

    aren->AddActor(outline.Get());
    aren->AddActor(door.Get());
    aren->AddActor(window1.Get());
    aren->AddActor(window2.Get());
    aren->AddActor(klower1.Get());
    aren->AddActor(klower2.Get());
    aren->AddActor(klower3.Get());
    aren->AddActor(klower4.Get());
    aren->AddActor(klower5.Get());
    aren->AddActor(klower6.Get());
    aren->AddActor(klower7.Get());
    aren->AddActor(hood1.Get());
    aren->AddActor(hood2.Get());
    aren->AddActor(hood3.Get());
    aren->AddActor(hood4.Get());
    aren->AddActor(hood6.Get());
    aren->AddActor(cookingPlate.Get());
    aren->AddActor(filter.Get());
    aren->AddActor(lines.Get());
    aren->AddActor(rake.Get());

    aren->SetBackground(colors->GetColor3d("SlateGray").GetData());

    vtkNew<vtkCamera> aCamera;
    aren->SetActiveCamera(aCamera.Get());
    aren->ResetCamera();

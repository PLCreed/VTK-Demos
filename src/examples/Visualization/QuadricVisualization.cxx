...
void CreatePlanes(vtkSampleFunction *function, vtkActor *actor, unsigned numberOfPlanes);

void CreateContours(vtkSampleFunction *function, vtkActor *actor, unsigned numberOfPlanes, unsigned numberOfContours);
...
int main(int, char *[])
{
    ...
    sample->SetImplicitFunction(quadric.Get());

    vtkNew<vtkActor> isoActor;
    CreateIsosurface(sample.Get(), isoActor.Get());
    vtkNew<vtkActor> outlineIsoActor;
    CreateOutline(sample.Get(), outlineIsoActor.Get());

    vtkNew<vtkActor> planesActor;
    CreatePlanes(sample.Get(), planesActor.Get(), 3);
    vtkNew<vtkActor> outlinePlanesActor;
    CreateOutline(sample.Get(), outlinePlanesActor.Get());
    planesActor->AddPosition(isoActor->GetBounds()[0] * 2.0, 0, 0);
    outlinePlanesActor->AddPosition(isoActor->GetBounds()[0] * 2.0, 0, 0);

    vtkNew<vtkActor> contourActor;
    CreateContours(sample.Get(), contourActor.Get(), 3, 15);
    vtkNew<vtkActor> outlineContourActor;
    CreateOutline(sample.Get(), outlineContourActor.Get());
    contourActor->AddPosition(isoActor->GetBounds()[0] * 4.0, 0, 0);
    outlineContourActor->AddPosition(isoActor->GetBounds()[0] * 4.0, 0, 0);

    renderer->AddActor(planesActor.Get());
    renderer->AddActor(outlinePlanesActor.Get());
    renderer->AddActor(contourActor.Get());
    renderer->AddActor(outlineContourActor.Get());
    renderer->AddActor(isoActor.Get());
    renderer->AddActor(outlineIsoActor.Get());
...
namespace
{
void CreateIsosurface(vtkSampleFunction *function, vtkActor *actor, unsigned int numberOfContours)
{
    double range[2];
    ...
    actor->SetMapper(contourMapper.Get());
    return;
}
...
void CreatePlanes(vtkSampleFunction *function, vtkActor *actor, unsigned int numberOfPlanes)
{
    ...
    for (unsigned int i = 0; i < numberOfPlanes; ++i) {
        ...
        extract->SetVOI(0, dims[0] - 1, 0, dims[1] - 1, slice + sliceIncr, slice + sliceIncr);
    ...
    actor->SetMapper(planesMapper.Get());
    actor->GetProperty()->SetAmbient(1.);
    return;
}

void CreateContours(vtkSampleFunction *function, vtkActor *actor, unsigned int numberOfPlanes,

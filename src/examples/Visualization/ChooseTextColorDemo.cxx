...
void ChooseContrastingColor(double *rgbIn, double *rgbOut, double threshold = .5,
                            const std::string &lightColor = "white", const std::string &darkColor = "black");

void ViewportBorder(vtkRenderer *renderer, double *color, bool last = false);

void RandomHSV(double hsv[3], double const &min_r, double const &max_r, vtkMinimalStandardRandomSequence *rng);
...
int main(int argc, char *argv[])
{
    ...
    for (auto i = 0; i < static_cast<int>(xGridDimensions * yGridDimensions); ++i) {
        ...
        double hsv[3];
        RandomHSV(hsv, min_r, max_r, rng.Get());
        double rgb[3];
        vtkMath::HSVToRGB(hsv, rgb);
        renderer->SetBackground(rgb);
        // Compute a good color for text on the renderer background
        ChooseContrastingColor(renderer->GetBackground(), rgb, threshold, lightColor, darkColor);
    ...
    for (auto row = 0; row < static_cast<int>(yGridDimensions); row++) {
        for (auto col = 0; col < static_cast<int>(xGridDimensions); col++) {
            ...
            // (xmin, ymin, xmax, ymax)
            double viewport[4] = {
                static_cast<double>(col) * rendererSize / (xGridDimensions * rendererSize),
                static_cast<double>(yGridDimensions - (row + 1)) * rendererSize / (yGridDimensions * rendererSize),
                static_cast<double>(col + 1) * rendererSize / (xGridDimensions * rendererSize),
                static_cast<double>(yGridDimensions - row) * rendererSize / (yGridDimensions * rendererSize)};
...
void ViewportBorder(vtkRenderer *renderer, double *color, bool last)
{
    ...
    cells->InsertNextCell(lines.Get());

    // now make tge polydata and display it
    vtkNew<vtkPolyData> poly;
    poly->Initialize();
    poly->SetPoints(points.Get());
    poly->SetLines(cells.Get());
    ...
    mapper->SetTransformCoordinate(coordinate.Get());
...
void ChooseContrastingColor(double *rgbIn, double *rgbOut, double threshold, const std::string &lightColor,

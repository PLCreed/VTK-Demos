...
public:
    ...
    void Execute(vtkObject *caller, unsigned long evId, void *) override
    {
        ...
        // triggered it.
        std::cout << interactor->GetClassName() << "  Event Id: " << evId << std::endl;
...
std::vector<std::string>::iterator FindParameter(std::string const &p, std::vector<std::string> &v);
...
int main(int argc, char *argv[])
{
    ...
    if (p != parameters.end()) {
        std::cerr << "Usage:   " << argv[0] << " [-p #] [-s #] [r #] [-c #]" << std::endl;
        ...
        std::cerr << "                 3 final configuration and save images" << std::endl;
    ...
    ren->SetActiveCamera(camera.Get());
    ...
    ren->AddActor(table.Get());
    table->SetMapper(tableMapper.Get());
    ...
    for (auto i = 0; i < numberOfPucks; ++i) {
        ...
        puck[i]->SetMapper(puckMapper.Get());
    ...
    if (configuration == 3) {
        Screenshot("hanoi0.png", renWin.Get());
    }
    if (configuration != 1) {
        // Begin recursion.
        Hanoi(renWin.Get(), pegStack, numberOfPucks - 1, 0, 2, 1);
        Hanoi(renWin.Get(), pegStack, 1, 0, 1, 2);
        if (!gotFigure2) {
            Hanoi(renWin.Get(), pegStack, numberOfPucks - 1, 2, 1, 0);

            renWin->Render();
            if (configuration == 3) {
                Screenshot("hanoi2.png", renWin.Get());
            }
        }
        // Report output.
        cout << "Number of moves: " << numberOfMoves << "\n"
             << "Polygons rendered each frame: " << 3 * 8 + 1 + numberOfPucks * (2 + puckResolution) << "\n"
    ...
    iren->AddObserver(vtkCommand::EndInteractionEvent, getOrientation.Get());
...
void MovePuck(vtkRenderWindow *renWin, PegArray &pegStack, int peg1, int peg2)
{
    ...
    auto distance = (H - (L * (static_cast<int>(pegStack[peg1].size()) - 1)) + rMax) / numberOfSteps;
    ...
    for (auto i = 0; i < numberOfSteps; i++) {
        ...
        if (numberOfMoves == 13 && i == 3) // for making book image
        {
            if (configuration == 3 || configuration == 2) {
                vtkCamera *cam = renWin->GetRenderers()->GetFirstRenderer()->GetActiveCamera();
                ...
                renWin->GetRenderers()->GetFirstRenderer()->SetActiveCamera(camera1.Get());
                renWin->Render();
                if (configuration == 3) {
    ...
    distance = ((L * (static_cast<int>(pegStack[peg2].size()) - 1)) - H - rMax) / numberOfSteps;
...
void PrintCameraOrientation(vtkCamera *cam)
{
    ...
    std::cout << setw(width) << "Position: " << CommaSeparatedList(std::vector<double>(pos, pos + 3)) << std::endl;
    std::cout << setw(width) << "Focal point: " << CommaSeparatedList(std::vector<double>(fp, fp + 3)) << std::endl;
    std::cout << setw(width) << "Clipping range: " << CommaSeparatedList(std::vector<double>(cr, cr + 2)) << std::endl;
    std::cout << setw(width) << "View up: " << CommaSeparatedList(std::vector<double>(vu, vu + 3)) << std::endl;

...
{
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " inputFilename(.vtp) [numberOfCuts]" << std::endl;
    ...
    plane->SetOrigin((bounds[1] + bounds[0]) / 2.0, (bounds[3] + bounds[2]) / 2.0, (bounds[5] + bounds[4]) / 2.0);
    ...
    reader->GetOutput()->GetPointData()->SetScalars(scalars.Get());
    ...
    cutter->GenerateValues(numberOfCuts, .99 * reader->GetOutput()->GetPointData()->GetScalars()->GetRange()[0],
    ...
    cutterActor->SetMapper(cutterMapper.Get());
    ...
    renderer->AddActor(cutterActor.Get());
    renderer->AddActor(modelActor.Get());

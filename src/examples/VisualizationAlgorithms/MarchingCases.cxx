...
int main(int argc, char *argv[])
{
    ...
    if (mcCases.size() == 1) {
        ...
        if (!mcCases.empty()) {
            ...
            std::copy(mcCases.begin(), mcCases.end() - 1, std::ostream_iterator<int>(ss, ", "));
            ss << mcCases.back();
    ...
    gridSize = static_cast<int>(mcCases.size()) < 4 ? static_cast<int>(mcCases.size()) : gridSize;
    ...
    for (size_t i = 0; i < mcCases.size(); ++i) {
        ...
        triangleEdgeActor->SetMapper(triangleEdgeMapper.Get());
        triangleEdgeActor->GetProperty()->SetDiffuseColor(color->GetColor3d("lamp_black").GetData());
        ...
        Triangles->SetMapper(aMapper.Get());
        Triangles->GetProperty()->SetDiffuseColor(color->GetColor3d("banana").GetData());
        ...
        CubeEdges->SetMapper(TubeMapper.Get());
        CubeEdges->GetProperty()->SetDiffuseColor(color->GetColor3d("khaki").GetData());
        ...
        ThresholdIn->SetInputData(Grid.Get());
        ...
        CubeVertices->SetMapper(SphereMapper.Get());
        CubeVertices->GetProperty()->SetDiffuseColor(color->GetColor3d("tomato").GetData());
        ...
        if (label) {
            ...
            labelTransform->SetTransform(aLabelTransform.Get());
        ...
        mcCase = mcCases[i];
        if (mcCase < 0) {
            cases[-mcCase](Scalars.Get(), caseLabel.Get(), 0, 1);
        } else {
            cases[mcCase](Scalars.Get(), caseLabel.Get(), 1, 0);
        }
        // Force the grid to update.
        Grid->Modified();

        // Add the actors to the renderer
        renderers[i]->AddActor(triangleEdgeActor.Get());
        renderers[i]->AddActor(base.Get());
        if (label) {
            renderers[i]->AddActor(labelActor.Get());
        }
        renderers[i]->AddActor(CubeEdges.Get());
        renderers[i]->AddActor(CubeVertices.Get());
        renderers[i]->AddActor(Triangles.Get());
    ...
    int xGridDimensions = static_cast<int>(mcCases.size()) < 4 ? static_cast<int>(mcCases.size()) : 4;
    int yGridDimensions = (static_cast<int>(mcCases.size()) - 1) / 4 + 1;
    std::cout << "Grid dimensions: (x, y): (" << xGridDimensions << ", " << yGridDimensions << ")" << std::endl;
    ...
    for (int row = 0; row < yGridDimensions; row++) {
        for (int col = 0; col < xGridDimensions; col++) {
            ...
            // (xmin, ymin, xmax, ymax)
            double viewport[4] = {static_cast<double>(col) / xGridDimensions,
...
std::string ProgramParameters(std::string const &programName)
{
    std::string ret = "Usage: " + programName + " [n [cases [cases ...]] [rotation] [label]\n";

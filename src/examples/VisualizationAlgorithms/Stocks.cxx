...
namespace
{
void AddStock(std::vector<vtkSmartPointer<vtkRenderer>> renderers, char *filename, std::string name, double &zPosition,
              bool useTubes);
}
...
int main(int argc, char *argv[])
{
    ...
    renderers.push_back(topRenderer.Get());
    renderers.push_back(bottomRenderer.Get());
    ...
    for (int i = 1; i < argc - 1; ++i) {
        AddStock(renderers, argv[i], vtksys::SystemTools::GetFilenameWithoutExtension(argv[i]), zPosition, useTubes);
    }
              ...
              bool useTubes)
{
    ...
    for (size_t r = 0; r < renderers.size(); ++r) {
        ...
        StockActor->SetMapper(StockMapper.Get());

        renderers[r]->AddActor(StockActor.Get());
        renderers[r]->AddActor(LabelActor.Get());

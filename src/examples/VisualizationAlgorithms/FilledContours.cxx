...
{
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " InputPolyDataFile(.vtp) NumberOfContours e.g filledContours.vtp 10"
                  << std::endl;
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < numberOfContours; i++) {
        ...
        clippersHi[i]->GetOutput()->GetCellData()->SetScalars(cd.Get());
    ...
    contourLineActor->SetMapper(contourLineMapperer.Get());
    ...
    // Add the actors
    ren1->AddActor(contourActor.Get());
    ren1->AddActor(contourLineActor.Get());

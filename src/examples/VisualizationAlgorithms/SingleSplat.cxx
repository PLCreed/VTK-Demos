...
int main(int, char *[])
{
    ...
    pData->SetPoints(pts.Get());
    pData->SetVerts(verts.Get());
    pData->GetPointData()->SetNormals(norms.Get());
    pData->GetPointData()->SetScalars(scalars.Get());

    // Splat point and generate isosurface
    vtkNew<vtkGaussianSplatter> splat;
    splat->SetInputData(pData.Get());
    ...
    splatActor->SetMapper(splatMapper.Get());
    ...
    aren->AddActor(splatActor.Get());
    aren->AddActor(outlineActor.Get());
    aren->AddActor(coneActor.Get());

 ...
 */
vtkSmartPointer<vtkPropAssembly> MakeCubeActor(std::array<double, 3> &scale, std::array<std::string, 3> &xyzLabels,
...
int main(int argc, char *argv[])
{
    ...
    auto axes = MakeCubeActor(scale, xyzLabels, colors.Get());
    vtkNew<vtkOrientationMarkerWidget> om;
    om->SetOrientationMarker(axes.Get());
    // Position upper left in the viewport.
    om->SetViewport(0.0, 0.8, 0.2, 1.0);
    om->SetInteractor(iren.Get());
    om->EnabledOn();
    om->InteractiveOn();

    // Right, Anterior, Superior.
    std::array<double, 3> scale1 {
        {1.5, 1.5, 1.5}
    };
    auto axes1 = MakeCubeActor(scale1, xyzLabels, colors.Get());
    vtkNew<vtkOrientationMarkerWidget> om1;
    om1->SetOrientationMarker(axes1.Get());
    // Position lower left in the viewport.
    om1->SetViewport(0, 0, 0.2, 0.2);
    om1->SetInteractor(iren.Get());
    om1->EnabledOn();
    om1->InteractiveOn();

    // Left, Posterior, Superior.
    std::array<double, 3> scale2 {
        {-1.5, -1.5, 1.5}
    };
    auto axes2 = MakeCubeActor(scale2, xyzLabels, colors.Get());
    vtkNew<vtkOrientationMarkerWidget> om2;
    om2->SetOrientationMarker(axes2.Get());
    // Position lower right in the viewport.
    om2->SetViewport(0.8, 0, 1.0, 0.2);
    om2->SetInteractor(iren.Get());
    om2->EnabledOn();
    om2->InteractiveOn();

    // Finally create an annotated cube actor adding it into an orientation marker
    // widget.
    auto axes3 = MakeAnnotatedCubeActor(colors.Get());
    vtkNew<vtkOrientationMarkerWidget> om3;
    om3->SetOrientationMarker(axes3.Get());
    // Position upper right in the viewport.
    om3->SetViewport(0.8, 0.8, 1.0, 1.0);
    om3->SetInteractor(iren.Get());
    om3->EnabledOn();
    om3->InteractiveOn();
    ...
    humanActor->SetMapper(humanMapper.Get());
    ...
    ren->AddActor(humanActor.Get());

    // Make the planes.
    auto actors = MakePlanesActors(colors.Get());
    for (auto const &actor : actors) {
        ren->AddViewProp(actor.Get());
    }
                                               ...
                                               vtkNamedColors *colors)
{
    ...
    assembly->AddPart(axes.Get());
    assembly->AddPart(cube.Get());
    return assembly;
}

vtkSmartPointer<vtkTransformPolyDataFilter> MakePlane(std::array<int, 2> &resolution, std::array<double, 3> &origin,
                                                      ...
                                                      std::array<double, 4> &wxyz, std::array<double, 3> &translate)
{
    ...
    vtkSmartPointer<vtkTransformPolyDataFilter> tpdPlane = vtkSmartPointer<vtkTransformPolyDataFilter>::New();

    tpdPlane->SetTransform(trnf.Get());
...
std::vector<vtkSmartPointer<vtkActor>> MakePlanesActors(vtkNamedColors *colors)
{
    ...
    std::array<double, 4> wxyz0 {
        {0, 0, 0, 0}
    };
    std::array<double, 3> translate {
        {-0.5, -0.5, 0}
    };
    std::array<double, 4> wxyz1 {
        {-90, 1, 0, 0}
    };
    std::array<double, 4> wxyz2 {
        {-90, 0, 1, 0}
    };
    ...
    actors[0]->GetProperty()->SetColor(colors->GetColor3d("SeaGreen").GetData());    // Transverse plane
    actors[1]->GetProperty()->SetColor(colors->GetColor3d("DeepSkyBlue").GetData()); // Coronal plane
    actors[2]->GetProperty()->SetColor(colors->GetColor3d("Tomato").GetData());      // Saggital plane
    return actors;
}
...
std::vector<vtkSmartPointer<vtkActor>> AddTextToPlanes()
{
    ...
    tpdPlane1->SetTransform(trnf1.Get());
    ...
    textActor1->SetMapper(textMapper1.Get());
    textActor1->SetScale(scale.data());
    textActor1->AddPosition(0.4, 0.49, 0.01);
    textActors.push_back(textActor1.Get());
    ...
    tpdPlane2->SetTransform(trnf2.Get());
    ...
    textActor2->SetMapper(textMapper2.Get());
    textActor2->SetScale(scale.data());
    textActor2->AddPosition(0.4, -0.49, -0.01);
    textActors.push_back(textActor2.Get());
    ...
    tpdPlane3->SetTransform(trnf3.Get());
    ...
    textActor3->SetMapper(textMapper3.Get());
    textActor3->SetScale(scale.data());
    textActor3->AddPosition(-0.01, 0.49, 0.4);
    textActors.push_back(textActor3.Get());
    ...
    tpdPlane4->SetTransform(trnf4.Get());
    ...
    textActor4->SetMapper(textMapper4.Get());
    textActor4->SetScale(scale.data());
    textActor4->AddPosition(0.01, -0.49, 0.4);
    textActors.push_back(textActor4.Get());
    ...
    tpdPlane5->SetTransform(trnf5.Get());
    ...
    textActor5->SetMapper(textMapper5.Get());
    textActor5->SetScale(scale.data());
    textActor5->AddPosition(0.49, 0.01, 0.20);
    textActors.push_back(textActor5.Get());
    ...
    tpdPlane6->SetTransform(trnf6.Get());
    ...
    textActor6->SetMapper(textMapper6.Get());
    textActor6->SetScale(scale.data());
    textActor6->AddPosition(-0.49, -0.01, 0.3);
    textActors.push_back(textActor6.Get());

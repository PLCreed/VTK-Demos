...
public:
    ...
    void Execute(vtkObject * /*caller*/, unsigned long, void *) override { this->actor->VisibilityOn(); }
...
{
    if (argc < 3) {
        std::cout << "Usage: " << argv[0] << " filename1 filename2 [numOfStreamLines] [illustration]" << std::endl;
        std::cout << "where: filename1 is combxyz.bin and filename2 is combq.bin." << std::endl;
        std::cout << "       numOfStreamLines is the number of streamlines, default 25" << std::endl;
    ...
    lineWidget->GetPolyData(seeds.Get());
    if (illustration) {
    ...
    lineWidget->SetInteractor(iren.Get());

    // Associate the line widget with the interactor and setup the callbacks.
    vtkNew<LWCallback> GenerateStreamlines;
    GenerateStreamlines->polyData = seeds.Get();
    GenerateStreamlines->renWin = renWin.Get();
    vtkNew<EnableActorCallback> BeginInteraction;
    BeginInteraction->actor = streamline.Get();
    lineWidget->AddObserver(vtkCommand::StartInteractionEvent, BeginInteraction.Get());
    lineWidget->AddObserver(vtkCommand::InteractionEvent, GenerateStreamlines.Get());
    ...
    lineWidget2->GetPolyData(seeds2.Get());
    lineWidget2->SetKeyPressActivationValue('L');
    lineWidget2->SetAlignToZAxis();
    lineWidget->ClampToBoundsOn();
    lineWidget2->PlaceWidget();

    lineWidget2->SetInteractor(iren.Get());

    // Associate the line widget with the interactor and setup the callbacks.
    vtkNew<LWCallback> GenerateStreamlines2;
    GenerateStreamlines2->polyData = seeds2.Get();
    GenerateStreamlines2->renWin = renWin.Get();
    vtkNew<EnableActorCallback> BeginInteraction2;
    BeginInteraction2->actor = streamline2.Get();
    lineWidget2->AddObserver(vtkCommand::StartInteractionEvent, BeginInteraction2.Get());
    lineWidget2->AddObserver(vtkCommand::InteractionEvent, GenerateStreamlines2.Get());

    // Here we set up two streamlines.
    vtkNew<vtkRungeKutta4> rk4;
    vtkNew<vtkStreamTracer> streamer;
    streamer->SetInputData(pl3d_output);
    streamer->SetSourceData(seeds.Get());
    streamer->SetMaximumPropagation(100);
    streamer->SetInitialIntegrationStep(0.2);
    streamer->SetIntegrationDirectionToForward();
    streamer->SetComputeVorticity(1);
    streamer->SetIntegrator(rk4.Get());
    ...
    streamline->SetMapper(streamMapper.Get());
    streamline->VisibilityOff();

    vtkNew<vtkStreamTracer> streamer2;
    streamer2->SetInputData(pl3d_output);
    streamer2->SetSourceData(seeds2.Get());
    ...
    streamer2->SetIntegrator(rk4.Get());
    ...
    streamline2->SetMapper(streamMapper2.Get());
    ...
    ren->AddActor(outlineActor.Get());
    ren->AddActor(streamline.Get());
    ren->AddActor(streamline2.Get());

...
namespace
{
void MakeXWidget(vtkSliderWidget *, vtkPointSource *, vtkSphereSource *, vtkRenderer *, vtkRenderWindowInteractor *);
void MakeYWidget(vtkSliderWidget *, vtkPointSource *, vtkSphereSource *, vtkRenderer *, vtkRenderWindowInteractor *);
void MakeZWidget(vtkSliderWidget *, vtkPointSource *, vtkSphereSource *, vtkRenderer *, vtkRenderWindowInteractor *);
} // namespace

int main(int argc, char *argv[])
{
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << "  geometry.wrl velocity.vtu e.g. room_vis.wrl fire_ug.vtu" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    vrmlImport->SetRenderWindow(renderWindow.Get());
    ...
    isoSurfaceActor->SetMapper(isoSurfaceMapper.Get());
    ...
    renderer->AddActor(tubesActor.Get());
    renderer->AddActor(sphereActor.Get());
    renderer->AddActor(isoSurfaceActor.Get());
    ...
    MakeXWidget(xWidget.Get(), seeds.Get(), sphere.Get(), renderer.Get(), renderWindowInteractor.Get());
    ...
    MakeYWidget(yWidget.Get(), seeds.Get(), sphere.Get(), renderer.Get(), renderWindowInteractor.Get());
    ...
    MakeZWidget(zWidget.Get(), seeds.Get(), sphere.Get(), renderer.Get(), renderWindowInteractor.Get());
...
void MakeXWidget(vtkSliderWidget *widget, vtkPointSource *pointSource, vtkSphereSource *sphereSource,
                 vtkRenderer *renderer, vtkRenderWindowInteractor *interactor)
{
    ...
    widget->SetRepresentation(sliderRepX.Get());
...
void MakeYWidget(vtkSliderWidget *widget, vtkPointSource *pointSource, vtkSphereSource *sphereSource,
                 vtkRenderer *renderer, vtkRenderWindowInteractor *interactor)
{
    // Setup a slider widget for each varying parameter
    double tubeWidth(.005);
    ...
    widget->SetRepresentation(sliderRepY.Get());
    widget->SetAnimationModeToAnimate();
    widget->SetNumberOfAnimationSteps(10);
    widget->SetInteractor(interactor);
    widget->EnabledOn();
...
void MakeZWidget(vtkSliderWidget *widget, vtkPointSource *pointSource, vtkSphereSource *sphereSource,
                 vtkRenderer *renderer, vtkRenderWindowInteractor *interactor)
{
    // Setup a slider widget for each varying parameter
    double tubeWidth(.005);
    ...
    widget->SetRepresentation(sliderRepZ.Get());
    widget->SetAnimationModeToAnimate();
    widget->SetNumberOfAnimationSteps(10);
    widget->SetInteractor(interactor);
    widget->EnabledOn();

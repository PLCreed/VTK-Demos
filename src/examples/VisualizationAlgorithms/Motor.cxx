...
int main(int argc, char *argv[])
{
    if (argc < 3) {
        ...
        std::cout << "where: textureFile is the texture file: texThres2.vtk." << std::endl;
    ...
    std::vector<std::string> partColours {"cold_grey", "peacock", "raw_sienna", "banana", "peach_puff"};
    ...
    for (auto i = 0; i < numberOfParts; ++i) {
        ...
        byuActor[i]->SetTexture(texture.Get());
        byuActor[i]->GetProperty()->SetColor(colors->GetColor3d(partColours[i]).GetData());

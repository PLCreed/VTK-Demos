...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
        std::cout << "Usage: " << argv[0] << " coarseMesh fineMesh e.g. coarseGrid.e fineGrid.e-s002" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    coarseActor->SetMapper(coarseMapper.Get());
    coarseActor->GetProperty()->SetEdgeVisibility(true);

    vtkNew<vtkRenderer> coarseRenderer;
    coarseRenderer->AddViewProp(coarseActor.Get());
    ...
    fineGeometryActor->SetMapper(fineGeometryMapper.Get());
    fineGeometryActor->GetProperty()->SetEdgeVisibility(true);

    vtkNew<vtkRenderer> fineGeometryRenderer;
    fineGeometryRenderer->AddViewProp(fineGeometryActor.Get());
    ...
    coarseInterpolator->SetKernel(kernel.Get());
    coarseInterpolator->SetLocator(locator.Get());
    ...
    coarseInterpolatorActor->SetMapper(coarseInterpolatorMapper.Get());
    coarseInterpolatorActor->GetProperty()->SetEdgeVisibility(true);

    vtkNew<vtkRenderer> coarseInterpolatorRenderer;
    coarseInterpolatorRenderer->AddActor(coarseInterpolatorActor.Get());
    ...
    coarseInterpolateAttributesActor->SetMapper(coarseInterpolateAttibutesMapper.Get());
    ...
    coarseInterpolateAttributesRenderer->AddActor(coarseInterpolateAttributesActor.Get());
    ...
    // Add the renderers
    window->AddRenderer(coarseRenderer.Get());                      // low res
    window->AddRenderer(coarseInterpolatorRenderer.Get());          // interpolated geometry
    window->AddRenderer(coarseInterpolateAttributesRenderer.Get()); // interpolated attributes
    window->AddRenderer(fineGeometryRenderer.Get());                // hi res with point data

...
int main(int, char *[])
{
    ...
    clipper->SetInputData(rgrid.Get());
    clipper->InsideOutOn();
    ...
    geometryFilter->SetInputData(rgrid.Get());
    ...
    clipperOutsideActor->GetProperty()->SetColor(colors->GetColor3d("Banana").GetData());
    ...
    std::cout << "The clipped dataset(inside) contains a " << std::endl
              << clipper->GetOutput()->GetClassName() << " that has " << numberOfCells << " cells" << std::endl;
    ...
    for (auto c : cellMap) {
        std::cout << "\tCell type " << vtkCellTypes::GetClassNameFromTypeId(c.first) << " occurs " << c.second
                  << " times." << std::endl;
    }
    ...
    std::cout << "The clipped dataset(outside) contains a " << std::endl
              << clipper->GetClippedOutput()->GetClassName() << " that has " << numberOfCells << " cells" << std::endl;

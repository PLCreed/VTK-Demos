...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        std::cout << "Usgae: " << argv[0] << " file.vtp e.g. Torso.vtp" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    normals->Update();
    normals->GetOutput()->GetPointData()->SetNormals(reader->GetOutput()->GetPointData()->GetNormals());
    ...
    for (it->InitTraversal(); !it->IsDoneWithTraversal() && numCells < numOriginalCells;
         it->GoToNextCell(), ++numCells) {}
    std::cout << "Num original: " << numOriginalCells << ", Num new: " << numNewCells
    ...
    connectivity->SetInputData(holePolyData.Get());
    ...
    filledActor->SetMapper(filledMapper.Get());
    filledActor->GetProperty()->SetDiffuseColor(colors->GetColor3d("Peacock").GetData());
    ...
    originalActor->SetBackfaceProperty(backfaceProp.Get());
    ...
    renderer->AddActor(filledActor.Get());

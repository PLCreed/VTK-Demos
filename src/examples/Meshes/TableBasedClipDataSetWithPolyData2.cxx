...
int main(int, char *[])
{
    ...
    clipPlanes->SetNormals(normals.Get());
    clipPlanes->SetPoints(clipPts.Get());

    vtkNew<vtkTableBasedClipDataSet> clipper;
    clipper->SetClipFunction(clipPlanes.Get());
    clipper->SetInputData(image.Get());
...
vtkSmartPointer<vtkRectilinearGrid> makeImage(int n)
{
    constexpr double cubeSize = 20.0; // physical linear dimension of entire system
    ...
    map->SetLookupTable(lut.Get());
    map->SetOutputFormatToRGBA();
    map->SetInputData(image0.Get());
    ...
    vtkSmartPointer<vtkRectilinearGrid> rectgrid = vtkSmartPointer<vtkRectilinearGrid>::New();
    ...
    rectgrid->SetXCoordinates(xcoords.Get());
    rectgrid->SetYCoordinates(ycoords.Get());
    rectgrid->SetZCoordinates(zcoords.Get());

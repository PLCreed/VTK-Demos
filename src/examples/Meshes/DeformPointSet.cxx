...
int main(int argc, char *argv[])
{
    ...
    pts->SetPoint(0, bounds[0] - .1 * (bounds[1] - bounds[0]), (bounds[3] + bounds[2]) / 2.0,
                  (bounds[5] + bounds[4]) / 2.0);
    pts->SetPoint(1, bounds[1] + .1 * (bounds[1] - bounds[0]), (bounds[3] + bounds[2]) / 2.0,
                  (bounds[5] + bounds[4]) / 2.0);
    pts->SetPoint(2, (bounds[1] + bounds[0]) / 2.0, bounds[2] - .1 * (bounds[3] - bounds[2]),
                  (bounds[5] + bounds[4]) / 2.0);
    pts->SetPoint(3, (bounds[1] + bounds[0]) / 2.0, bounds[3] + .1 * (bounds[3] - bounds[2]),
                  (bounds[5] + bounds[4]) / 2.0);
  ...
  tris->InsertCellPoint(3);  tris->InsertCellPoint(0);  tris->InsertCellPoint(5);
    ...
    pd->SetPolys(tris.Get());

    // Display the control mesh
    vtkNew<vtkPolyDataMapper> meshMapper;
    meshMapper->SetInputData(pd.Get());
    vtkNew<vtkActor> meshActor;
    meshActor->SetMapper(meshMapper.Get());
    ...
    deform->SetInputData(input.Get());
    deform->SetControlMeshData(pd.Get());
    ...
    pts->SetPoint(5, controlPoint[0], controlPoint[1], bounds[5] + .8 * (bounds[5] - bounds[4]));
    pts->Modified();
    ...
    polyActor->SetMapper(polyMapper.Get());
    ...
    renderer->AddActor(polyActor.Get());

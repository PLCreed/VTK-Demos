...
{
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " file.vtp e.g ExportBunny.vtp" << std::endl;
        return EXIT_FAILURE;
    }
...
void ImportMultiBlockScene(vtkRenderer *renderer, std::string fileName)
{
    ...
    reader->Update();
    std::cout << "Importing " << dynamic_cast<vtkMultiBlockDataSet *>(reader->GetOutput())->GetNumberOfBlocks()
              << " actors" << std::endl;

#if VTK890
    vtkDataObjectTree *input = dynamic_cast<vtkDataObjectTree *>(reader->GetOutput());

    using Opts = vtk::DataObjectTreeOptions;
    for (vtkDataObject *dso : vtk::Range(input, Opts::SkipEmptyNodes | Opts::VisitOnlyLeaves)) {
    ...
    for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem()) {
        ...
        actor->SetBackfaceProperty(backProperty.Get());
        RestorePropertyFromFieldData("BackfaceProperty", actor->GetBackfaceProperty(), pd);
        RestoreActorFromFieldData("Actor", actor.Get(), pd);
        renderer->AddActor(actor.Get());
    }
#endif
}

void RestoreCameraFromFieldData(std::string const &arrayPrefix, vtkCamera *camera, vtkPolyData *pd)
{
...
void RestorePropertyFromFieldData(std::string const &arrayPrefix, vtkProperty *property, vtkPolyData *pd)
{
    if (property) {
        vtkFieldData *fd = pd->GetFieldData();
        ...
        property->SetSpecularPower(
            fd->GetArray(std::string(arrayPrefix + ":" + "SpecularPower").c_str())->GetTuple1(0));
        property->SetEdgeColor(fd->GetArray(std::string(arrayPrefix + ":" + "EdgeColor").c_str())->GetTuple(0));
        property->SetEdgeVisibility(
            fd->GetArray(std::string(arrayPrefix + ":" + "EdgeVisibility").c_str())->GetTuple1(0));
        // property->SetVertexColor(fd->GetArray(std::string(arrayPrefix + ":" + "VertexColor").c_str())->GetTuple(0));
        // property->SetVertexVisibility(
        //     fd->GetArray(std::string(arrayPrefix + ":" + "VertexVisibility").c_str())->GetTuple1(0));
        property->SetInterpolation(
            fd->GetArray(std::string(arrayPrefix + ":" + "Interpolation").c_str())->GetTuple1(0));
        property->SetOpacity(fd->GetArray(std::string(arrayPrefix + ":" + "Opacity").c_str())->GetTuple1(0));
        property->SetRepresentation(
            fd->GetArray(std::string(arrayPrefix + ":" + "Representation").c_str())->GetTuple1(0));
        property->SetBackfaceCulling(
            fd->GetArray(std::string(arrayPrefix + ":" + "BackfaceCulling").c_str())->GetTuple1(0));
        property->SetFrontfaceCulling(
            fd->GetArray(std::string(arrayPrefix + ":" + "FrontfaceCulling").c_str())->GetTuple1(0));
        ...
        property->SetLineStipplePattern(
            fd->GetArray(std::string(arrayPrefix + ":" + "LineStipplePattern").c_str())->GetTuple1(0));
        property->SetLineStippleRepeatFactor(
            fd->GetArray(std::string(arrayPrefix + ":" + "LineStippleRepeatFactor").c_str())->GetTuple1(0));
        ...
        property->SetRenderPointsAsSpheres(
            fd->GetArray(std::string(arrayPrefix + ":" + "RenderPointsAsSpheres").c_str())->GetTuple1(0) == 1.0 ?
                true :
                false);
        property->SetRenderLinesAsTubes(
            fd->GetArray(std::string(arrayPrefix + ":" + "RenderLinesAsTubes").c_str())->GetTuple1(0) == 1.0 ? true :
                                                                                                               false);
        property->SetShading(
...
void RestoreActorFromFieldData(std::string const &arrayPrefix, vtkActor *actor, vtkPolyData *pd)
{
    vtkFieldData *fd = pd->GetFieldData();

    actor->SetDragable(fd->GetArray(std::string(arrayPrefix + ":" + "Dragable").c_str())->GetTuple1(0) == 1.0 ? true :
                                                                                                                false);
    actor->SetPickable(fd->GetArray(std::string(arrayPrefix + ":" + "Pickable").c_str())->GetTuple1(0) == 1.0 ? true :
                                                                                                                false);
    ...
    actor->SetForceOpaque(
        fd->GetArray(std::string(arrayPrefix + ":" + "ForceOpaque").c_str())->GetTuple1(0) == 1.0 ? true : false);
    actor->SetForceTranslucent(
        fd->GetArray(std::string(arrayPrefix + ":" + "ForceTranslucent").c_str())->GetTuple1(0) == 1.0 ? true : false);
}
} // namespace

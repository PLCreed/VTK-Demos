...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3) {
        std::cout << "Required arguments: Filename Actorname e.g. teapot.wrl teapot" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    importer->Read();
    importer->SetRenderWindow(renderWindow.Get());
    importer->Update();

    // ----------------------------------------------------------
    vtkObject *defActor = importer->GetVRMLDEFObject(argv[2]);
    if (defActor == NULL) {
        std::cout << "Cannot locate actor " << argv[2] << " in " << filename << std::endl;

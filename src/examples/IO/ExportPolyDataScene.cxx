...
void SaveMapperAsFieldData(std::string const &, vtkPolyDataMapper *, vtkPolyData *);
void SaveActorAsFieldData(std::string const &, vtkActor *, vtkPolyData *);
void SaveLookupTableAsFieldData(std::string const &, vtkScalarsToColors *, vtkPolyData *);
} // namespace
...
int main(int argc, char *argv[])
{
    ...
    for (int c = 0; c < 4; ++c) {
        for (int b = 0; b < 4; ++b) {
            for (int a = 0; a < 4; ++a) {
                ...
                if (b == 1) {
                    actor->GetProperty()->SetDiffuseColor(colors->GetColor3d("Orchid").GetData());
                } else if (b == 2) {
                    actor->GetProperty()->SetDiffuseColor(colors->GetColor3d("Salmon").GetData());
                } else if (b == 3) {
                    actor->GetProperty()->SetDiffuseColor(colors->GetColor3d("SandyBrown").GetData());
                } else {
                ...
                if (c == 3) {
                    ...
                    actor->SetUserTransform(rotateTransform.Get());
                } else {
                    actor->GetProperty()->SetInterpolationToFlat();
                }
                actor->AddPosition(1.5 * a * delta[0], 1.5 * b * delta[1], 1.5 * c * delta[2]);
    ...
    if (argc < 2) {
        ...
        prefix = "Export" + vtksys::SystemTools::GetFilenameWithoutExtension(argv[1]) + ".vtp";
    }
    std::cout << "Scene is exported to " << vtksys::SystemTools::GetCurrentWorkingDirectory() + "/" + prefix
              << std::endl;
...
namespace
{
void ExportMultiBlockScene(vtkRenderer *renderer, std::string fileName, bool binary)
{
    ...
    for (vtkIdType a = 0; a < actors->GetNumberOfItems(); ++a) {
        ...
        multiBlockDataset->SetBlock(a, pd.Get());

        // Save Camera
        SaveCameraAsFieldData("Camera", renderer->GetActiveCamera(), pd.Get());

        // Save Property
        SavePropertyAsFieldData("Property", actor->GetProperty(), pd.Get());
        SavePropertyAsFieldData("BackfaceProperty", actor->GetBackfaceProperty(), pd.Get());
        // Save Mapper
        SaveMapperAsFieldData("PolyDataMapper", dynamic_cast<vtkPolyDataMapper *>(actor->GetMapper()), pd.Get());
        // Save Actor
        SaveActorAsFieldData("Actor", actor, pd.Get());
    }
    ...
    writer->SetInputData(multiBlockDataset.Get());
...
void SaveCameraAsFieldData(std::string const &arrayPrefix, vtkCamera *camera, vtkPolyData *pd)
{
...
void SavePropertyAsFieldData(std::string const &arrayPrefix, vtkProperty *property, vtkPolyData *pd)
{
    if (property) {
...
void SaveMapperAsFieldData(std::string const &arrayPrefix, vtkPolyDataMapper *mapper, vtkPolyData *pd)
{
    if (mapper->GetLookupTable()) {
        ...
        pd->GetFieldData()->AddArray(Table.Get());
...
void SaveActorAsFieldData(std::string const &arrayPrefix, vtkActor *actor, vtkPolyData *pd)
{

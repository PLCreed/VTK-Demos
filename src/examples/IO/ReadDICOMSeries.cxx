...
public:
    ...
    void SetStatusMapper(vtkTextMapper *statusMapper) { _StatusMapper = statusMapper; }
...
int main(int argc, char *argv[])
{
    ...
    sliceTextMapper->SetTextProperty(sliceTextProp.Get());

    vtkNew<vtkActor2D> sliceTextActor;
    sliceTextActor->SetMapper(sliceTextMapper.Get());
    ...
    usageTextMapper->SetTextProperty(usageTextProp.Get());

    vtkNew<vtkActor2D> usageTextActor;
    usageTextActor->SetMapper(usageTextMapper.Get());
    ...
    myInteractorStyle->SetImageViewer(imageViewer.Get());
    myInteractorStyle->SetStatusMapper(sliceTextMapper.Get());

    imageViewer->SetupInteractor(renderWindowInteractor.Get());
    // make the interactor use our own interactorstyle
    // cause SetupInteractor() is defining it's own default interatorstyle
    // this must be called after SetupInteractor()
    renderWindowInteractor->SetInteractorStyle(myInteractorStyle.Get());
    // add slice status message and usage hint message to the renderer
    imageViewer->GetRenderer()->AddActor2D(sliceTextActor.Get());
    imageViewer->GetRenderer()->AddActor2D(usageTextActor.Get());

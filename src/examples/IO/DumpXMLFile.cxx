...
vtkDataSet *ReadAnXMLFile(const char *fileName)
{
    ...
    reader->Update();
    reader->GetOutput()->Register(reader.Get());
...
int main(int argc, char *argv[])
{
    ...
    while (f < argc) {
        ...
        std::string extension = vtksys::SystemTools::GetFilenameLastExtension(argv[f]);
        ...
        if (pd) {
            ...
            for (int i = 0; i < pd->GetNumberOfArrays(); i++) {
                std::cout << "\tArray " << i << " is named " << (pd->GetArrayName(i) ? pd->GetArrayName(i) : "NULL")
                          ...
                          << " of type " << pd->GetArray(i)->GetClassName() << std::endl;
            }
        }
        ...
        if (dataSet->GetFieldData()) {
            ...
            for (int i = 0; i < dataSet->GetFieldData()->GetNumberOfArrays(); i++) {
                std::cout << "\tArray " << i << " is named " << dataSet->GetFieldData()->GetArray(i)->GetName()
                          ...
                          << " of type " << dataSet->GetFieldData()->GetArray(i)->GetClassName() << std::endl;
            }
        }
        f++;
    }

...
int main(int, char *[])
{
    ...
    CreateColorImage(image1.Get(), 1, 0);

    vtkNew<vtkImageSliceMapper> imageSliceMapper1;
    imageSliceMapper1->SetInputData(image1.Get());

    vtkNew<vtkImageSlice> imageSlice1;
    imageSlice1->SetMapper(imageSliceMapper1.Get());
    imageSlice1->GetProperty()->SetOpacity(.5);

    // Image 2
    vtkNew<vtkImageData> image2;
    CreateColorImage(image2.Get(), 4, 1);

    vtkNew<vtkImageSliceMapper> imageSliceMapper2;
    imageSliceMapper2->SetInputData(image2.Get());

    vtkNew<vtkImageSlice> imageSlice2;
    imageSlice2->SetMapper(imageSliceMapper2.Get());
    imageSlice2->GetProperty()->SetOpacity(.5);

    // Stack
    vtkNew<vtkImageStack> imageStack;
    imageStack->AddImage(imageSlice1.Get());
    imageStack->AddImage(imageSlice2.Get());
    // imageStack->SetActiveLayer(1);

    // Setup renderers
    vtkNew<vtkRenderer> renderer;
    renderer->AddViewProp(imageStack.Get());

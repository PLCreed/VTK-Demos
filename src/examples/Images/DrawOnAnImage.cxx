...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::cout << "Usage: " << argv[0] << " InputImageFilename e.g. Gourds2.jpg" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    int radius;
    radius =
        (image->GetExtent()[1] < image->GetExtent()[3]) ? image->GetExtent()[1] * 2 / 5 : image->GetExtent()[3] * 2 / 5;
    ...
    drawing->FillBox(image->GetExtent()[0], image->GetExtent()[1], image->GetExtent()[2], image->GetExtent()[3]);
    ...
    imageViewer->GetRenderer()->SetBackground(colors->GetColor3d("LightSlateGray").GetData());

...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::cout << "Usage: " << argv[0] << " Filename(.jpg) e.g Gourds2.jpg" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    lookupTable->SetTableValue(1,
                               colors->GetColor4d("Khaki").GetData()); // label 1 is opaque and colored
    lookupTable->Build();

    vtkNew<vtkImageMapToColors> mapTransparency;
    mapTransparency->SetLookupTable(lookupTable.Get());
    mapTransparency->PassAlphaToOutputOn();
    mapTransparency->SetInputData(maskImage.Get());
    ...
    renderer->AddActor(maskActor.Get());

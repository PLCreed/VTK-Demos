...
int main(int argc, char *argv[])
{
    ...
    gradientFilter->SetInputData(image.Get());
    ...
    gradientFilter->GetOutput()->GetPointData()->AddArray(zeroes.Get());

    std::string scalarName = gradientFilter->GetOutput()->GetPointData()->GetScalars()->GetName();
    ...
    vectorGradientActor->SetMapper(vectorGradientMapper.Get());
    vectorGradientActor->GetProperty()->SetColor(namedColors->GetColor3d("Tomato").GetData());
    ...
    vectorGradientRenderer->SetBackground(namedColors->GetColor3d("Bkg").GetData());

    vtkNew<vtkRenderWindow> renderWindow;
    renderWindow->SetSize(1000, 250);
    renderWindow->AddRenderer(originalRenderer.Get());
    renderWindow->AddRenderer(xGradientRenderer.Get());
    renderWindow->AddRenderer(yGradientRenderer.Get());
    renderWindow->AddRenderer(vectorGradientRenderer.Get());
    ...
    originalActor->GetMapper()->SetInputData(originalImage.Get());
    ...
    xGradientRenderer->AddActor(xGradientActor.Get());
    yGradientRenderer->AddActor(yGradientActor.Get());
    vectorGradientRenderer->AddActor(vectorGradientActor.Get());
    vectorGradientRenderer->AddActor(originalActor.Get());

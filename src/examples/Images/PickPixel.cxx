...
{
public:
    static vtkImageInteractionCallback1 *New() { return new vtkImageInteractionCallback1; }
    ...
    void SetAnnotation(vtkCornerAnnotation *annotation) { this->Annotation = annotation; }
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::cout << "Usage: " << argv[0] << " Filename e.g. Gourds2.jpg" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    cornerAnnotation->GetTextProperty()->SetColor(colors->GetColor3d("LightGoldenrodYellow").GetData());

    imageViewer->GetRenderer()->AddViewProp(cornerAnnotation.Get());

    // Callback listens to MouseMoveEvents invoked by the interactor's style
    vtkNew<vtkImageInteractionCallback1> callback;
    callback->SetViewer(imageViewer.Get());
    callback->SetAnnotation(cornerAnnotation.Get());
    callback->SetPicker(propPicker.Get());
    ...
    imageStyle->AddObserver(vtkCommand::MouseMoveEvent, callback.Get());

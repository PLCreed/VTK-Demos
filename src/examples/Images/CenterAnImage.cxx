...
public:
    ...
    static vtkBorderCallback *New() { return new vtkBorderCallback; }
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        std::cerr << "Usage: " << argv[0] << " Required parameters: Filename e.g. Ox.jpg" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    borderCallback->SetRenderer(renderer.Get());
...
void vtkBorderCallback::Execute(vtkObject *caller, unsigned long, void *)
{
    ...
    std::cout << "Lower left coordinate:  " << fmt(lowerLeft[0]) << "," << fmt(lowerLeft[1]) << "," << fmt(lowerLeft[2])
              << std::endl;
    ...
    std::cout << "Upper right coordinate: " << fmt(upperRight[0]) << "," << fmt(upperRight[1]) << ","
              << fmt(upperRight[2]) << std::endl;

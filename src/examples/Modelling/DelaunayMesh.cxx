...
int main(int, char *[])
{
    ...
    profile->SetPoints(points.Get());

    // Perform a 2D Delaunay triangulation on them.
    vtkNew<vtkDelaunay2D> delny;
    delny->SetInputData(profile.Get());
    delny->SetTolerance(0.001);
    vtkNew<vtkPolyDataMapper> mapMesh;
    mapMesh->SetInputConnection(delny->GetOutputPort());
    vtkNew<vtkActor> meshActor;
    meshActor->SetMapper(mapMesh.Get());
    meshActor->GetProperty()->SetColor(colors->GetColor3d("MidnightBlue").GetData());
    ...
    edgeActor->SetMapper(mapEdges.Get());
    ...
    ballActor->SetMapper(mapBalls.Get());
    ...
    ren->AddActor(ballActor.Get());
    ren->AddActor(edgeActor.Get());

...
namespace
{
vtkSmartPointer<vtkDataSet> ReadFinancialData(const char *fname, const char *x, const char *y, const char *z,
                                              const char *s);
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " financial_file e.g. financial.txt" << endl;
        return EXIT_FAILURE;
    }
    char *fname = argv[1];
    ...
    std::array<unsigned char, 4> popColor {
        {230, 230, 230, 255}
    };
    colors->SetColor("PopColor", popColor.data());

    // read data
    auto dataSet = ReadFinancialData(fname, "MONTHLY_PAYMENT", "INTEREST_RATE", "LOAN_AMOUNT", "TIME_LATE");
    // construct pipeline for original population
    vtkNew<vtkGaussianSplatter> popSplatter;
    popSplatter->SetInputData(dataSet.Get());
    ...
    lateActor->SetMapper(lateMapper.Get());
    ...
    axesActor->SetMapper(axesMapper.Get());
    ...
    // set up renderer
    renderer->AddActor(lateActor.Get());
    renderer->AddActor(axesActor.Get());
    renderer->AddActor(popActor.Get());
...
namespace
{
vtkSmartPointer<vtkDataSet> ReadFinancialData(const char *filename, const char *x, const char *y, const char *z,
                                              const char *s)
{
    float xyz[3];
    FILE *file;
    ...
    if (!ParseFile(file, x, xV) || !ParseFile(file, y, yV) || !ParseFile(file, z, zV) || !ParseFile(file, s, sV)) {
    ...
    dataSet->SetPoints(newPts.Get());
    dataSet->GetPointData()->SetScalars(newScalars.Get());

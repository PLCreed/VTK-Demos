...
void MakeWidget(vtkSliderWidget *, vtkDelaunay3D *, vtkTextMapper *, vtkRenderer *, vtkRenderWindowInteractor *);
} // namespace
...
int main(int argc, char *argv[])
{
    ...
    originalMapper->SetInputData(polyData.Get());
    originalMapper->ScalarVisibilityOff();
    originalMapper->SetScalarModeToUseCellData();

    vtkNew<vtkActor> originalActor;
    originalActor->SetMapper(originalMapper.Get());
    ...
    cleaner->SetInputData(polyData.Get());
    ...
    delaunayActor->SetMapper(delaunayMapper.Get());
    ...
    delaunay3DAlpha->GetOutput()->GetCellData()->SetScalars(cellData.Get());
    ...
    delaunayAlphaActor->SetMapper(delaunayAlphaMapper.Get());
    ...
    renderWindow->AddRenderer(delaunayRenderer.Get());
    delaunayRenderer->SetViewport(centerViewport);

    renderWindow->AddRenderer(delaunayAlphaRenderer.Get());
    ...
    MakeWidget(widget.Get(), delaunay3DAlpha.Get(), textMapper.Get(), delaunayAlphaRenderer.Get(),
               renderWindowInteractor.Get());

    originalRenderer->AddActor(originalActor.Get());
    delaunayRenderer->AddActor(delaunayActor.Get());
    delaunayAlphaRenderer->AddActor(delaunayAlphaActor.Get());
    delaunayAlphaRenderer->AddViewProp(textActor.Get());
...
{
public:
    static SliderCallbackAlpha *New() { return new SliderCallbackAlpha; }
    ...
    virtual void Execute(vtkObject *caller, unsigned long, void *)
    {
        ...
        this->Delaunay3D->GetOutput()->GetCellData()->SetScalars(cellData.Get());
        std::stringstream ss;
        ss << "numTetras: " << numTetras << std::endl;
        ss << "numLines: " << numLines << std::endl;
        ss << "numTris: " << numTris << std::endl;
...
void MakeWidget(vtkSliderWidget *widget, vtkDelaunay3D *delaunay3D, vtkTextMapper *textMapper, vtkRenderer *renderer,
                vtkRenderWindowInteractor *interactor)
{
    ...
    widget->SetRepresentation(sliderRepAlpha.Get());
    ...
    widget->AddObserver(vtkCommand::InteractionEvent, callbackAlpha.Get());
}

...
public:
     ...
     * never be called.
     */
    static void AddCallback(const char *id, LogHandlerCallbackT callback, void *user_data, Verbosity verbosity,
                            CloseHandlerCallbackT on_close = nullptr, FlushHandlerCallbackT on_flush = nullptr);
...
#if !defined(__WRAP__)
    static void LogF(Verbosity verbosity, const char *fname, unsigned int lineno, VTK_FORMAT_STRING_TYPE format, ...)
        VTK_PRINTF_LIKE(4, 5);
    static void StartScopeF(Verbosity verbosity, const char *id, const char *fname, unsigned int lineno,
    ...
    public:
        ...
        LogScopeRAII(LogScopeRAII &&other) : Internals(other.Internals) { other.Internals = nullptr; }
#else
...
#define vtkVLogF(level, ...)                                                                                           \
    ((level) > vtkLogger::GetCurrentVerbosityCutoff()) ? (void)0 :                                                     \

...
class PlotRasterItemBase::PrivateData
{
public:
    PrivateData() : alpha(-1), paintAttributes(PlotRasterItemBase::PaintInDeviceResolution)
    {
        cache.policy = PlotRasterItemBase::NoCache;
    }

    int alpha;

    PlotRasterItemBase::PaintAttributes paintAttributes;

    uint renderThreadCount = 6;

    struct ImageCache
    {
        PlotRasterItemBase::CachePolicy policy;
        QRectF area;
        QSizeF size;
        QImage image;
    } cache;
...
static QRectF qwtStripRect(const QRectF &rect, const QRectF &area, const ScaleMap &xMap, const ScaleMap &yMap,
                           const Interval &xInterval, const Interval &yInterval)
{
    QRectF r = rect;
    if (xInterval.borderFlags() & Interval::ExcludeMinimum) {
    ...
    if (xInterval.borderFlags() & Interval::ExcludeMaximum) {
    ...
    if (yInterval.borderFlags() & Interval::ExcludeMinimum) {
    ...
    if (yInterval.borderFlags() & Interval::ExcludeMaximum) {
...
static QImage qwtExpandImage(const QImage &image, const ScaleMap &xMap, const ScaleMap &yMap, const QRectF &area,
                             const QRectF &area2, const QRectF &paintRect, const Interval &xInterval,
                             const Interval &yInterval)
{
    ...
    const QRectF r = ScaleMap::transform(xMap, yMap, area).normalized();
...
static void qwtTransformMaps(const QTransform &tr, const ScaleMap &xMap, const ScaleMap &yMap, ScaleMap &xxMap,
                             ScaleMap &yyMap)
{
...
static void qwtAdjustMaps(ScaleMap &xMap, ScaleMap &yMap, const QRectF &area, const QRectF &paintRect)
{
...
static bool qwtUseCache(PlotRasterItemBase::CachePolicy policy, const QPainter *painter)
{
    bool doCache = false;

    if (policy == PlotRasterItemBase::PaintCache) {
...
// ! Constructor
PlotRasterItemBase::PlotRasterItemBase(const QString &)
{
    init();
}

// ! Destructor
PlotRasterItemBase::~PlotRasterItemBase()
{
    delete m_data;
}

void PlotRasterItemBase::init()
{
 ...
 */
void PlotRasterItemBase::setPaintAttribute(PaintAttribute attribute, bool on)
{
    if (on) {
 ...
 */
bool PlotRasterItemBase::testPaintAttribute(PaintAttribute attribute) const
{
...
/*!
   ...
   \sa alpha()
 */
void PlotRasterItemBase::setAlpha(int alpha)
{
    if (alpha < 0) {
        alpha = -1;
    }
...
/*!
   ...
   \sa setAlpha()
 */
int PlotRasterItemBase::alpha() const
{
    return m_data->alpha;
}
 ...
 */
void PlotRasterItemBase::setCachePolicy(PlotRasterItemBase::CachePolicy policy)
{
 ...
 */
PlotRasterItemBase::CachePolicy PlotRasterItemBase::cachePolicy() const
{
...
/*!
   ...
   \sa setCachePolicy()
 */
void PlotRasterItemBase::invalidateCache()
{
 ...
 */
QRectF PlotRasterItemBase::pixelHint(const QRectF &area) const
{
    Q_UNUSED(area);
    return QRectF();
}
 ...
 */
void PlotRasterItemBase::draw(QPainter *painter, const ScaleMap &xMap, const ScaleMap &yMap,
                              const QRectF &canvasRect) const
{
    ...
    if (br.isValid() && !br.contains(area)) {
        ...
        paintRect = ScaleMap::transform(xxMap, yyMap, area);
    }
    ...
    if (pixelRect.isEmpty()) {
        if (/*QwtPainter::roundingAlignment(painter)*/ true) {
        ...
        if (imageRect != paintRect) {
            const QRect r(qRound(imageRect.x() - paintRect.x()), qRound(imageRect.y() - paintRect.y()),
        ...
        if ((imageArea.right() == xInterval.maxValue()) && !(xInterval.borderFlags() & Interval::ExcludeMaximum)) {
            imageArea.adjust(0, 0, pixelRect.width(), 0);
        }
        if ((imageArea.bottom() == yInterval.maxValue()) && !(yInterval.borderFlags() & Interval::ExcludeMaximum)) {
    ...
    // QwtPainter::drawImage(painter, imageRect, image);
    {
        ...
        if (alignedRect != imageRect) {
            const QRectF clipRect = imageRect.adjusted(0.0, 0.0, -1.0, -1.0);
 ...
 */
Interval PlotRasterItemBase::interval(Qt::Axis axis) const
{
    Q_UNUSED(axis);
    return Interval();
}
...
/*!
   ...
   \sa PlotRasterItemBase::interval()
 */
QRectF PlotRasterItemBase::boundingRect() const
{
...
void PlotRasterItemBase::setRenderThreadCount(uint numThreads)
{
    m_data->renderThreadCount = numThreads;
}

uint PlotRasterItemBase::renderThreadCount() const
{
    return m_data->renderThreadCount;
}

QImage PlotRasterItemBase::compose(const ScaleMap &xMap, const ScaleMap &yMap, const QRectF &imageArea,
                                   const QRectF &paintRect, const QSize &imageSize, bool doCache) const
{
    QImage image;
    ...
    if (image.isNull()) {
        ...
        const ScaleMap xxMap = imageMap(Qt::Horizontal, xMap, imageArea, imageSize, dx);
        ...
        const ScaleMap yyMap = imageMap(Qt::Vertical, yMap, imageArea, imageSize, dy);
 ...
 */
ScaleMap PlotRasterItemBase::imageMap(Qt::Orientation orientation, const ScaleMap &map, const QRectF &area,
